...
# import h5py
# from cdi_dislo.general_utilities.utils import (nan_to_zero,    zero_to_nan,)
...
def setup_phase_retrieval(
    ...
    support=None,
    support_size=None,
    support_threshold="0.10, 0.30",
    support_threshold_method="rms",
    support_only_shrink=False,
    support_update_period=0,
    ...
    roi="full",
    mpi="run",
    print_params=True,  # New parameter to control whether to print parameters
):
    """
...
def setup_preprocessing(
    preprocess_shape=[
        200,
        200,
    ],  # Required, 2 or 3 values. If 2, will take the whole RC
...
def get_het_normal_strain(
    ...
    displacement_gradient = get_displacement_gradient(
        displacement, voxel_size, gradient_method
    )
    displacement_gradient = np.moveaxis(
        np.asarray(displacement_gradient), source=0, destination=3
    )
...
# ------------------------------------------------------------------------------------------------------------
def hybrid_gradient(
    data: np.ndarray, d0: float, d1: float, d2: float
...
# ------------------------------------------------------------------------------------------------------------
def get_lattice_parametre(
    nrj, gamma, delta, cch, cy, cz, scale=1, hkl=[1, 1, 1], s_g=1, s_d=1
):
    """
...
# ------------------------------------------------------------------------------------------------------------
def phase_offset_to_zero_clement(
    phase: np.ndarray,
...
def getting_strain_mapvti(
    ...
    if save_filename_vti:
        ...
        gu.save_to_vti(
            ...
            tuple_array=(
                nan_to_zero(modulus),
                nan_to_zero(phase_0),
                zero_to_nan(phase_1),  # type: ignore
    ...
    if plot_debug:
        plot_2D_slices_middle_one_array3D(
            ...
            vmax=3,
            cmap="jet",
            fig_title="original phase midlle slice",
        )
        if save_filename_vti:
        ...
        plot_2D_slices_middle_one_array3D(
            ...
            vmax=3,
            cmap="jet",
            fig_title=f"phase + {np.round(i_phase, 4)} midlle slice",
        )  # type: ignore
        if save_filename_vti:
        ...
        plot_2D_slices_middle_one_array3D(
            ...
            vmax=0.3,
            cmap="jet",
            fig_title="original phase gradient midlle slice",
        )
        if save_filename_vti:
        ...
        plot_2D_slices_middle_one_array3D(
            ...
            vmax=0.3,
            cmap="jet",
            fig_title=f"phase  + {np.round(i_phase, 4)} gradient midlle slice",
        )  # type: ignore
        if save_filename_vti:
