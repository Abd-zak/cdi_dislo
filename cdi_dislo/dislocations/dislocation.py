...
def compare_dislocations_v0(
    ...
    Ux_iso_mid, Uy_iso_mid, Uz_iso_mid = (
        Ux_iso[:, :, z_mid],
        Uy_iso[:, :, z_mid],
        Uz_iso[:, :, z_mid],
    )
    Ux_aniso_mid, Uy_aniso_mid, Uz_aniso_mid = (
        Ux_aniso[:, :, z_mid],
        Uy_aniso[:, :, z_mid],
        Uz_aniso[:, :, z_mid],
    )
    ...
    if trig_plot_x:
        ...
        for r_val in r_values:
            ...
            Ux_aniso_sorted = remove_large_jumps_alter_unwrap(
                (Ux_aniso_sorted), threshold=0.1
            )  # type: ignore

            (h1,) = ax.plot(
                theta_sorted,
                center_angles(Ux_iso_sorted),
                marker1,
                label=f"Isotropic r={r_val}",
            )
            (h2,) = ax.plot(
                (theta_sorted_ani),
                center_angles(Ux_aniso_sorted),
                marker2,
    ...
    if trig_plot_y:
        ...
        for r_val in r_values:
            ...
            sorted_idx = np.argsort(theta_masked)
            (h1,) = ax.plot(
                theta_masked[sorted_idx],
                center_angles(Uy_iso_masked[sorted_idx]),
                marker1,
                label=f"Isotropic r={r_val}",
            )
            (h2,) = ax.plot(
                theta_masked[sorted_idx],
                center_angles(Uy_aniso_masked[sorted_idx]),
                marker2,
                label=f"Anisotropic r={r_val}",
            )
            add_legend_entry(ax, h1, h1.get_label())
    ...
    if trig_plot_z:
        ...
        for r_val in r_values:
            ...
            (h1,) = ax.plot(
                theta_masked[sorted_idx],
                center_angles(Uz_iso_masked[sorted_idx]),
                marker1,
                label=f"Isotropic r={r_val}",
            )
            (h2,) = ax.plot(
                theta_masked[sorted_idx],
                center_angles(Uz_aniso_masked[sorted_idx]),
                marker2,
                label=f"Anisotropic r={r_val}",
            )
            add_legend_entry(ax, h1, h1.get_label())
...
def compare_dislocations_v1(
    ...
    def config_plot_diffu(ax, f_s, string_axis):
        ...
        ax.set_xlabel(r"$\theta$ $_{(rad)}$", fontsize=f_s, fontweight="bold")
        ax.set_ylabel(
            rf"$U_{{{string_axis}}}$  $_{{anis}}$ - $U_{{{string_axis}}}$ $_{{iso}}$ $_{{({unit})}}$",
            fontsize=f_s,
            fontweight="bold",
        )
        ax.tick_params(
    ...
    if trig_plot_x:
        ...
        for r_val in r_values:
            ...
            Ux_aniso_sorted = np.unwrap(
                Ux_aniso_sorted, period=jump_filer_period
            )
            ...
            ax2_x.plot(
                ...
                diff_iso_ani_x,
                "o",
                label=f"Anisotropic - Isotropic r={r_val}",
            )
    ...
    if trig_plot_y:
        ...
        for r_val in r_values:
            ...
            Uy_aniso_sorted = np.unwrap(
                Uy_aniso_sorted, period=jump_filer_period
            )
    ...
    if trig_plot_z:
        ...
        for r_val in r_values:
            ...
            Uz_aniso_sorted = np.unwrap(
                Uz_aniso_sorted, period=jump_filer_period
            )
...
def compare_dislocations_v2(
    ...
    def plot_component_comparison(
        ax_l, ax_r, U_iso, U_aniso, comp, center_angles_func
    ):
        for r_val in r_values:
            mask = np.isclose(R_mid, r_val, atol=1)
    ...
    R_mid = R[:, :, z_mid]
    Theta_mid = (
        np.unwrap(theta[:, :, z_mid]) if mod_theta else theta[:, :, z_mid]
    )
    ...
    for i, (comp, iso, aniso) in enumerate(active_comps):
        plot_component_comparison(
            axs[i, 0], axs[i, 1], iso, aniso, comp, center_angles
        )
...
def compare_metals(
    ...
    def plot_component(
        ...
        for i_metal in list_metals:
            ...
            for r_val in r_values:
                ...
                ref_sorted = np.unwrap(
                    ref_masked[sorted_idx], period=jump_filer_period
                )
        ...
        for ax, title, is_diff in zip(
            ...
            ax.yaxis.offsetText.set_fontsize(f_s)
            ax.set_xlabel(
                r"$\theta$ $_{(rad)}$", fontsize=f_s, fontweight="bold"
            )
    ...
    if trig_plot_x:
        ...
        plot_component(
            ax_main, ax_diff, Ux_dict, Ux_dict[ref_metal], r"$U_x$", "x"
        )
        row_idx += 1
    if trig_plot_y:
        ax_main, ax_diff = axs[row_idx]
        plot_component(
            ax_main, ax_diff, Uy_dict, Uy_dict[ref_metal], r"$U_y$", "y"
        )
        row_idx += 1
    if trig_plot_z:
        ax_main, ax_diff = axs[row_idx]
        plot_component(
            ax_main, ax_diff, Uz_dict, Uz_dict[ref_metal], r"$U_z$", "z"
        )
        row_idx += 1
...
def plot_displacement_fields(
    ...
    for i, (flag, U_mid, title, ylabel) in enumerate(
        ...
        for r_idx, r_val in enumerate(r_values):
            ...
            U_sorted = np.unwrap(
                U_masked[sorted_indices], period=jump_filer_period
            )
        ...
        ax.set_xlabel(
            r"$\theta$ $_{(\mathrm{rad})}$", fontsize=f_s, fontweight="bold"
        )
        ax.set_ylabel(ylabel, fontsize=f_s, fontweight="bold")
        ax.tick_params(axis="both", labelsize=f_s)
        ax.yaxis.offsetText.set_fontsize(f_s)
...
def edge_dislocation_anisotropic(bx, by, c11, c12, c44, r, theta):
    ...
    # Calculate A2
    A2 = (c11_bar_prime - c12_prime) / (
        2 * c11_bar_prime * lambda_ * np.sin(phi)
    )

    # Calculate A3
    A3 = (c11_bar_prime + c12_prime) / (
        2 * c11_bar_prime * lambda_ * np.cos(phi)
    )
...
def edge_dislocation_isotropic_cart(b, nu, x, y):
    ...
    U_x = (
        b
        / (2 * np.pi)
        * (np.arctan2(y, x) + (x * y) / (2 * (1 - nu) * (x**2 + y**2)))
    )
...
def mixed_dislocation_isotropic_cart(b_x, b_y, b_z, x, y, nu=0.3):
    ...
    U_x = (
        -b_x
        / (2 * np.pi)
        * (np.arctan2(y, x) + (x * y) / (2 * (1 - nu) * r_squared))
    )
    U_y = (
        -b_y
        / (2 * np.pi)
...
def edge_dislocation_anisotropic_carti_v0(bx, by, c11, c12, c44, x, y):
    ...
    # Calculate q^2 and t^2
    q_squared = (
        x**2 + ((lambda_**2) * (y**2)) + 2 * x * y * lambda_ * np.cos(phi)
    )
    t_squared = (
        x**2 + ((lambda_**2) * (y**2)) - 2 * x * y * lambda_ * np.cos(phi)
    )
...
# ------------------------------------------------------------------------------------------------------------
def automatic_object_roi(obj, threshold=0.1, factor=0.4, plot=False):
    module = np.abs(obj)
...
def get_cropped_module_phase(
    ...
    if support is None:
        if threshold_module is None:
            if obj.ndim == 3:
                threshold_module = 0.3  # Seems that 3D data need a smaller threshold. Nope in the end, .3 is fine
            if obj.ndim == 2:
...
#####################################################################################################################
# ------------------------------------------------------------------------------------------------------------
def process_and_merge_clusters_dislo_strain_map(
    data,
    amp,
    phase,
    save_path,
    ...
    print(
        f"Filtered clusters: {np.unique(filtered_clusters)[1:]}"
    ...
    for cluster_id in range(1, np.max(merged_clusters) + 1):  # type: ignore
        ...
        if add_random_noise:
            noise = np.random.normal(
                0, noise_stddev, size=cluster_indices.shape
            )
        ...
        if apply_polynomial_fit:
            poly_fit_x = np.polyfit(
                u_new, spline_points[:, 0], polynomial_degree
            )
            poly_fit_y = np.polyfit(
                u_new, spline_points[:, 1], polynomial_degree
            )
            poly_fit_z = np.polyfit(
                u_new, spline_points[:, 2], polynomial_degree
            )
            ...
            spline_points = np.vstack(
                [poly_points_x, poly_points_y, poly_points_z]
            ).T
        ...
        for point in spline_points:
            ...
            # Define bounds
            x_min, x_max = (
                int(np.floor(x_center - cylinder_radius)),
                int(np.ceil(x_center + cylinder_radius)),
            )
            y_min, y_max = (
                int(np.floor(y_center - cylinder_radius)),
                int(np.ceil(y_center + cylinder_radius)),
            )
            z_min, z_max = (
                int(np.floor(z_center - cylinder_radius)),
                int(np.ceil(z_center + cylinder_radius)),
            )

            # Clip bounds to array limits
            x_min, x_max = (
                max(x_min, 0),
                min(x_max, cylindrical_mask.shape[0] - 1),
            )
            y_min, y_max = (
                max(y_min, 0),
                min(y_max, cylindrical_mask.shape[1] - 1),
            )
            z_min, z_max = (
                max(z_min, 0),
                min(z_max, cylindrical_mask.shape[2] - 1),
            )
    ...
    cluster_sizes = [
        (label_id, np.sum(final_labeled_clusters == label_id))
    ...
    for old_label, new_label in label_mapping.items():
        sorted_labeled_clusters[final_labeled_clusters == old_label] = (
            new_label
        )

    final_labeled_clusters = sorted_labeled_clusters
    print(
        f"Final number of clusters after relabeling and sorting: {num_final_clusters}"
    )

    # Save data (if enabled)
    if save_output:
        save_as_vti(
            output_path=save_path + "_amp-phase_bulk_nan.vti",
            voxel_size=tuple(voxel_sizes),
            **{
                "density": nan_to_zero(amp),
                "phase": nan_to_zero(phase),
                "strain_amp": final_labeled_clusters,
            },
        )
    ...
    if debug_plot:
        ...
        cbar = fig.colorbar(
            scatter, ax=ax, shrink=0.5, aspect=10
        )  # Add colorbar
        ...
        gif_path = (
            save_path + "_final_dislocation_clustering_and_processing.gif"
        )
...
# ------------------------------------------------------------------------------------------------------------
def create_circular_mask_with_angles(
    data_shape, line_points, selected_point_index, r, dr
):
    """
    ...
    if selected_point_index == 0:
        ...
        z_axis = (
            line_points[selected_point_index + 1]
            - line_points[selected_point_index - 1]
        ) / 2
...
# ------------------------------------------------------------------------------------------------------------
def automatic_obj_slice_dilo_finding(
    obj_slice, threshold_module=0.005, plot=False
):
    module = np.abs(obj_slice)
    support = module > 0.2 * np.max(module)
...
def dislocation_reorientation(obj, normal, dislo_position, plot=False):
    from cdi_dislo.rotation.rotation import (
        rotation_matrix_from_vectors,
    )
    from scipy.interpolate import RegularGridInterpolator
    from cdi_dislo.ewen_utilities.plot_utilities import plot_2D_slices_middle
    from cdi_dislo.ewen_utilities.Global_utilities import apply_roi
    ...
    pos = [
        np.arange(obj.shape[axis]) - dislo_position[axis]
    ...
    obj_rot = apply_roi(obj_rot, automatic_object_roi(obj_rot, factor=0.2))
...
# ------------------------------------------------------------------------------------------------------------
def phase_around_dislo(
    phase, dislo_position, r0, dr=1, eleminate_zero_values=True, plot=False
):
    phase = nan_to_zero(phase)
...
def plot_compare_phase_radius(
    ...
    for radius in radius_list:
        angle_ring, phase_ring = phase_around_dislo(
            phase, dislo_position, radius
        )
...
# ------------------------------------------------------------------------------------------------------------
def remove_phase_ramp_dislo(
    phase,
    dislo_position,
    radius_1=6,
    radius_2=15,
    function_method="trigo",
    plot=True,
):
    """
    ...
    angle_ring_1, phase_ring_1 = phase_around_dislo(
        phase, dislo_position, radius_1, plot=plot
    )
    angle_ring_2, phase_ring_2 = phase_around_dislo(
        phase, dislo_position, radius_2, plot=plot
    )

    if plot:
        fig, ax = plt.subplots(2, 3, figsize=(12, 8))
        ax[0, 0].plot(
            angle_ring_1, phase_ring_1, ".", label=f"{round(radius_1, 1)}"
        )
        ax[0, 0].plot(
            angle_ring_2, phase_ring_2, ".", label=f"{round(radius_2, 1)}"
        )
    ...
    angle_ring = np.linspace(
        np.array([angle_ring_1.min(), angle_ring_2.min()]).max(),
        np.array([angle_ring_1.max(), angle_ring_2.max()]).min(),
        100,
    )
    ...
    if function_method == "trigo":
        popt, pcov = curve_fit(
            phase_difference_fit_function, angle_ring, difference
        )
        ...
        frequencies = fft.fftfreq(
            len(angle_ring), d=(angle_ring[1] - angle_ring[0])
        )
    ...
    if plot:
        ax[0, 1].plot(
            angle_ring, phase_ring_1, ".", label=f"{round(radius_1, 1)}"
        )  # type: ignore
        ax[0, 1].plot(
            angle_ring, phase_ring_2, ".", label=f"{round(radius_1, 1)}"
        )  # type: ignore
    ...
    if plot:
        ...
        angle_ring_1, phase_ring_1 = phase_around_dislo(
            phase_no_ramp, dislo_position, radius_1
        )
        angle_ring_2, phase_ring_2 = phase_around_dislo(
            phase_no_ramp, dislo_position, radius_2
        )
...
# ------------------------------------------------------------------------------------------------------------
def plot_compare_phase_radius_around_dislo(
    scan,
    amp,
    phase,
    ...
    fig, ax = plt.subplots(1, 1, figsize=(7, 5))
    for radius in radius_list:
        phase_ring, angle_ring, circular_mask = (
            plot_phase_around_dislo_one_radius_new(
                scan,
                amp,
                phase,
                selected_dislocation_data,
        ...
        ax.plot(
            ...
            y,
            "o",
            label=f"r:{round(radius, 1)}"
            + rf" $\Delta\phi$ {np.round(-y.min() + y.max(), 2)}rad",
        )
    ax.legend(title="distance from\ndislocation center")
    ax.set_xlabel("angle around dislocation (rad)", fontsize=15)
    ax.set_ylabel("phase (rad)", fontsize=15)
...
# ------------------------------------------------------------------------------------------------------------
def remove_phase_ramp_dislo_new(
    scan,
    amp,
    phase,
    selected_dislocation_data,
...
# ------------------------------------------------------------------------------------------------------------
def process_dislocation_scan(
    scan,
    files_data_ortho,
    scan_list,
    save_path,
    threshold_strain_amp=0.3,
    threshold_module=0.005,
    dislocation_axis=2,
    radius_1=3,
    radius_2=5,
    radius_min=2,
    ...
    plot_2D_slices_middle(
        obj, voxel_sizes=voxel_sizes, threshold_module=threshold_module
    )
    ...
    coord_dislo_all_filtered = [
        [
            tuple(ii)
            for ii in np.array(coord_dislo_all)[
                :, coord_dislo_all[dislocation_axis] == i
            ][
                :,
                np.where(
    ...
    obj_slice = obj[
        slice_coord_array_along_axis(obj, dislocation_axis, midle_slice)
    ]
    # strain_amp_slice = strain_amp[
    #     slice_coord_array_along_axis(obj, dislocation_axis, midle_slice)
    # ]

    plot_object_module_phase_2d(
        obj_slice, crop=False, threshold_module=threshold_module
    )
    ...
    module, phase = nan_to_zero(
        get_cropped_module_phase(
            obj_slice, threshold_module=threshold_module, crop=False
        )
    )  # type: ignore
    plt.show()
    ...
    angle_ring, phase_ring = phase_around_dislo(
        phase,
        dislo_position_slice,
        radius_1,
        dr=2,
        eleminate_zero_values=True,
        plot=True,
    )
    plt.suptitle(f"phase around dislo {scan}", y=1.05)
    plt.savefig(
        f"{save_path}{scan}_slice_{midle_slice}_phase_around_dislo.png"
    )
    plt.show()

    plot_compare_phase_radius(
        phase,
        dislo_position_slice,
    ...
    plt.suptitle(f"phase around dislo {scan}", y=1.05)
    plt.savefig(
        f"{save_path}{scan}_slice_{midle_slice}_phase_around_dislo_multipleradius.png"
    )
    plt.show()

    # Remove phase ramp
    phase_no_ramp = remove_phase_ramp_dislo(
        phase,
        dislo_position_slice,
        radius_1=radius_1,
        radius_2=radius_2,
    ...
    plt.savefig(
        f"{save_path}{scan}_slice_{midle_slice}_phase_ramp_around_dislo.png"
    )
    plt.show()
    ...
    plt.savefig(
        f"{save_path}{scan}_slice_{midle_slice}_rampedphase_around_dislo_multipleradius.png"
    )
    plt.show()
...
# ------------------------------------------------------------------------------------------------------------
def create_circular_mask_with_angles_new(
    data_shape,
    centroid,
    direction,
    selected_point_index,
...
# ------------------------------------------------------------------------------------------------------------
# ------------------------------------------------------------------------------------------------------------
def plot_compare_phase_radius_around_dislo_new(
    scan,
    amp,
    phase,
    selected_dislocation_data,
...
def remove_jumps_dbscan_algo(
    ...
    filtered_indices_dbscan = filtered_indices_cpd[
        mask_dbscan
    ]  # Track indices
...
def plot_phase_vs_angle(
    ...
    marker_map = {
        scan: markers[i % len(markers)]
        for i, scan in enumerate(data["scan"].unique())
    }
    ...
    )  # Change font size for y-axis ticks

    # Improved legend placement with dark blue background
    legend = ax.legend(
        title="Scan", fontsize=font_size, loc=1, ncol=ncol, frameon=True
    )
...
def deconvolute_1d(
    ...
    if method == "FFT":
        ...
        if impulse_response is None:
            raise ValueError(
                "Impulse response required for Wiener deconvolution"
            )
...
# ------------------------------------------------------------------------------------------------------------
def filter_outliers(
    x, y, method="zscore", threshold=3, window=11, polyorder=2, eps_dbscan=0.2
):
    """
    ...
    if method == "zscore":
        ...
        raise ValueError(
            "Unsupported method. Choose from 'zscore', 'iqr', 'dbscan', 'savgol', or 'polyfit'."
        )
...
def process_phase_ring_ortho_old(angle, phase, factor_phase=1):
    ...
    angle_ring, phase_ring = remove_large_jumps(
        angle_ring, phase_ring, threshold_factor=1.5
    )

    # Remove points with large jumps
    for _ in range(3):  # Repeat cleaning process multiple times
        trigger_new_period = np.array(
            np.where(np.abs(np.diff(phase_ring)) > 0.5)
        )[0]
        trigger_new_period = np.concatenate(
            (
                trigger_new_period,
                trigger_new_period + 1,
                trigger_new_period - 1,
            )
        )
    ...
    trigger_angle_rotation = np.where(
        (angle_final < 180) & (angle_final > -180)
    )
    angle_final = angle_final[trigger_angle_rotation]
    phase_final = phase_final[trigger_angle_rotation]

    phase_ring_1_smooth = savgol_filter(
        phase_final, window_length=50, polyorder=4
    )
...
def plot_dislocation_phase_analysis(
    ...
    coefficients_theo = [
        np.polyfit(theta_data, phase_theo_3_cases[i], 1)
    ...
    phase_diff_oscillation = np.array(
        [
            center_angles(
                ...
                - (
                    coefficients_theo[i][0] * theta_data
                    + coefficients_theo[i][1]
                )
            )
            for i in range(num_cases)
        ]
    ...
    phase_diff_oscillation_exp_based = np.array(
        [
            center_angles(
                phase_theo_3_cases[i]
                - (coef_exp[0] * theta_data + coef_exp[1])
            )
            for i in range(num_cases)
        ]
    )
    ...
    phase_diff = np.array(
        [
            center_angles(phase_theo_3_cases[i] - phase_data)
            for i in range(num_cases)
        ]
    )
    ...
    for i_b in range(num_cases):
        label = "".join(
            map(
                str,
                (
                    b_cases[i_b] / np.nanmin(zero_to_nan(np.abs(b_cases[i_b])))
                ).astype(int),
            )
        )
        ax0.plot(
    ...
    ax0.legend(
        fontsize=font_size - 2, loc="best", frameon=False, markerscale=1.2
    )
    ...
    ax1.set_title(
        f"$\phi$ Difference: Theory - {type_data_to_comp} {title_suffix}"
    )  # type: ignore
...
def apply_noisy_sigma_todata(x, y, sigma_perc=0.5, overfactor_x=5, plot=True):
    ...
    random_noise = np.random.normal(
        loc=0.0, scale=error_std, size=x_noisy.shape
    )
...
def get_predicted_theo_u(
    ...
    Ux_mixed_iso, Uy_mixed_iso, Uz_mixed_iso = (
        mixed_dislocation_isotropic_cart(b_x, b_y, b_z, x, y, nu=0.3)
    )
    ...
    (
        ...
        r, theta, Ux_mixed_iso, Uy_mixed_iso, Uz_mixed_iso, z_mid=0
    )
...
def function_to_predict_mixed_dislo_isotropic(x, y, b, bz, alpha):
    ...
    return (
        alpha * (bragg[0] * U_x + bragg[1] * U_y)
        - bragg[2] * (1 - alpha) * U_z
    )


# ------------------------------------------------------------------------------------------------------------
def get_predictiontheo_of_experimental(
    bx=1,
    by=1,
    bz=1,
    alpha_screw_edge=1,
    bragg=[1, 1, -1],
    metals_elastic_constants={},
):
    # a0 = 3.924
    ...
    Ux_edge_iso, Uy_edge_iso, Uz_edge_iso = edge_dislocation_isotropic(
        bz, nu, r, theta
    )
    Ux_screw_iso, Uy_screw_iso, Uz_screw_iso = screw_dislocation_isotropic(
        bz, theta
    )
    Ux_edge_aniso, Uy_edge_aniso, Uz_edge_aniso = (
        edge_dislocation_anisotropic_carti(bx, by, c11, c12, c44, x, y)
    )
    Ux_screw_aniso, Uy_screw_aniso, Uz_screw_aniso = (
        screw_dislocation_anisotropic_carti(bz, c11, c12, c44, x, y)
    )
    Theta_edge = np.unwrap(Theta + np.pi / 2)
    # Theta_edge=(Theta)
    (
        Ux_edge_aniso_masked_flatted,
        Uy_edge_aniso_masked_flatted,
        Uz_edge_aniso_masked_flatted,
    ) = (
        Ux_edge_aniso[:, :, z_mid][mask].flatten(),
        Uy_edge_aniso[:, :, z_mid][mask].flatten(),
        Uz_edge_aniso[:, :, z_mid][mask].flatten(),
    )
    (
        Ux_screw_aniso_masked_flatted,
        Uy_screw_aniso_masked_flatted,
        Uz_screw_aniso_masked_flatted,
    ) = (
        Ux_screw_aniso[:, :, z_mid][mask].flatten(),
        Uy_screw_aniso[:, :, z_mid][mask].flatten(),
        Uz_screw_aniso[:, :, z_mid][mask].flatten(),
    )
    ...
    thetaedge_flatten_sorted = np.rint(
        thetaedge_flatten[sorted_indices_edge] * 180 / np.pi
    )

    Ux_edge_aniso_masked_flatted_sorted = Ux_edge_aniso_masked_flatted[
        sorted_indices_edge
    ]
    Uy_edge_aniso_masked_flatted_sorted = Uy_edge_aniso_masked_flatted[
        sorted_indices_edge
    ]
    Uz_edge_aniso_masked_flatted_sorted = Uz_edge_aniso_masked_flatted[
        sorted_indices_edge
    ]
    Ux_screw_aniso_masked_flatted_sorted = Ux_screw_aniso_masked_flatted[
        sorted_indices
    ]
    Uy_screw_aniso_masked_flatted_sorted = Uy_screw_aniso_masked_flatted[
        sorted_indices
    ]
    Uz_screw_aniso_masked_flatted_sorted = Uz_screw_aniso_masked_flatted[
        sorted_indices
    ]

    u_final = alpha_screw_edge * (
        bragg[0] * Ux_edge_aniso_masked_flatted_sorted
        + bragg[1] * Uy_edge_aniso_masked_flatted_sorted
        + bragg[2] * Uz_edge_aniso_masked_flatted_sorted
    ) + (1 - alpha_screw_edge) * (
        bragg[0] * Ux_screw_aniso_masked_flatted_sorted
        + bragg[1] * Uy_screw_aniso_masked_flatted_sorted
        + bragg[2] * Uz_screw_aniso_masked_flatted_sorted
    )
...
# ------------------------------------------------------------------------------------------------------------
def get_abc_sixs2019_for_simu(
    save__orth, scan="S472", wanted_shape=[240, 256, 256]
):
    ...
    cx0_new, cy0_new, cz0_new = np.array(
        C_O_M(pad_to_shape(intens, cx.shape))
    ).astype("int")
...
def transform_and_discretize(
    ...
    def compute_voxel_grid(x_coords, y_coords, z_coords, target_voxel_size):
        ...
        """
        nx_p = int(
            (np.max(x_coords) - np.min(x_coords)) / target_voxel_size[0] / 10
        )
        ny_p = int(
            (np.max(y_coords) - np.min(y_coords)) / target_voxel_size[1] / 10
        )
        nz_p = int(
            (np.max(z_coords) - np.min(z_coords)) / target_voxel_size[2] / 10
        )
...
def adjust_phase_and_plot(
    ...
    detref, intensity_cropped = crop_3darray_pos(
        intensity, det_ref_return=True
    )
...
def inverse_diffraction_pattern(
    ...
    if center_diffraction:
        ...
        if shift_center is not None:
            try:
                center_pos = tuple(
                    np.array(center_pos) + np.array(shift_center)
                )
...
# ------------------------------------------------------------------------------------------------------------
def inverse_diffraction_pattern_old(
    reciprocal_space_object,
    center_diffraction=True,
    center_method="com",
    shift_center=None,
    ...
    if center_diffraction:
        ...
        reciprocal_space_object = pad_to_shape(
            reciprocal_space_object, shape_data_pad
        )
        ...
        if center_method == "com":
            ...
            print(
                "wrong value for center_method of fft intensity. the default value (com) will be taken"
            )
            crop_pos__ = com_pos
        ...
        if shift_center is not None:
            try:
                crop_pos__ = tuple(
                    np.array(crop_pos__) + np.array(shift_center)
                )
            except Exception:
                print("Could not apply the shift.")
...
# 🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀
def plot_debug_difraction_from_obj(
    amplitude,
    phase,
    intensity,
    phase_reciprocal,
    ...
    axes[1, 0].imshow(
        np.log1p(intensity_slice), cmap="inferno", extent=[-1, 1, -1, 1]
    )
    ...
    axes[1, 1].imshow(
        phase_reciprocal_slice, cmap="twilight", extent=[-1, 1, -1, 1]
    )
    ...
    axes[2, 0].imshow(
        reconstructed_amplitude_slice, cmap="gray", extent=[-1, 1, -1, 1]
    )
    ...
    axes[2, 1].imshow(
        reconstructed_phase_slice, cmap="twilight", extent=[-1, 1, -1, 1]
    )
    ...
    axes[3, 0].imshow(
        np.log1p(intensity_sum), cmap="inferno", extent=[-1, 1, -1, 1]
    )
...
def plot_3d_dislo_amp_disloline(
    ...
    verts2, faces2, _, _ = skm.marching_cubes(
        selected_dislocation_data, level=0.5
    )
...
# ------------------------------------------------------------------------------------------------------------
# 🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀
def plot_phase_subplots_allsimu(
    threecases_raw_results_simu_angle,
    threecases_raw_results_simu_phase,
    scan_list,
    title="Phase Data Subplots",
    save_path=None,
):
    """
    ...
    fig, axes = plt.subplots(
        nrows=num_rows, ncols=num_cols, figsize=(12, 4 * num_rows), sharey=True
    )
...
# 🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀
# ------------------------------------------------------------------------------------------------------------
def plot_phase_data_comparison_exp_to_theo_v0(
    exp_angle,
    exp_phase,
    theo_phases,
    labels,
    ...
    def preprocess_phase(theta, phi):
        """Subtracts linear + cos(θ) + sin(θ) + const to isolate harmonic component."""
...
def plot_phase_data_comparison_combined_simu_exp_to_theo(
    ...
    axs[0].set_title(
        "Experimental vs Theoretical", fontsize=16, fontweight="bold"
    )
    ...
    axs[1].set_title(
        "Simulation vs Theoretical", fontsize=16, fontweight="bold"
    )
    ...
    for i, label in enumerate(labels_simu):
        ...
        if minus_theta:
            y_simu, trend_simu_eq = remove_linear_fit(
                simu_angle_cut, simu_phase_cut
            )
...
def decompose_experimental_phase_new(theta, phi_exp):
    ...
    f_highfreq = coeffs_high[0] * np.cos(2 * theta) + coeffs_high[1] * np.sin(
        2 * theta
    )

    # Step 7: Combine linear parts
    f_linear = f_linear_first + f_linear_second
    coeffs_linear = np.array(
        [slope_first + slope_second, intercept_first + intercept_second]
    )

    # Step 8: Final outputs
    f_filterlowfreq = f_linear + f_lowfreq
    f_oscillation_final = center_angles(
        phi_exp - f_filterlowfreq
    )  # oscillations after removing linear + low
    f_fitoscillation_final = center_angles(
        f_highfreq
    )  # fitted high-frequency oscillations
...
# ------------------------------------------------------------------------------------------------------------
def find_coefficients_burgzervector(
    target_vector, basis_vectors=[[1, 0, 1], [1, -1, 0], [0, -1, 1]]
):
    """
    ...
    print(
        f"  {target_vector} = ({coefficients[0]:.4f}) * {basis_vectors[0]} + "
...
#############################################################################################################################################################################################################
def plot_residuals_with_optional_cos_sin_fit(
    scan_list,
    angle_list,
    phase_list,
    center_angles,
    ...
    for ii_scan, i_scan in enumerate(scan_list):
        ...
        if fit_cos_sin_2theta:
            ...
            coeffs_harmonic, *_ = np.linalg.lstsq(
                X_harmonic, residual, rcond=None
            )
            ...
            row.extend(
                [f"{coeffs_harmonic[0]:.3f}", f"{coeffs_harmonic[1]:.3f}"]
            )
        else:
            s2 = None
            row.extend(["–", "–"])
...
# ------------------------------------------------------------------------------------------------------------
def plot_individual_scans_with_table_in_last_subplot(
...
def plot_individual_scans_with_table_below(
    ...
    fig = plt.figure(
        figsize=(figsize_unit[0] * n_cols, figsize_unit[1] * total_rows)
    )
...
# 🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀
# 🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀
def plot_phase_data_comparison_exp_to_theo_second_version(
    exp_angle,
    exp_phase,
    theo_phases,
    labels,
...
def find_best_burgers_vectors_single_scan(
    ...
    for i_b, b in enumerate(b_scaled_list):
        predicted_phase = dislo_phase_model(
            theta, t, G_exp, b, align_theta=align_theta
        )

        pred_osc, pred_fit, _, f_linear, coeffs_linear = (
            decompose_experimental_phase(theta, predicted_phase)
        )
...
def compute_predictions_for_all_scans(
    ...
    index_s472 = list(all_results_exp_scan_list).index(
        preferred_reference_scan
    )
    theta_ref = all_results_exp_angle[index_s472]
    s472_results_exp_xdisloval, s472_results_exp_xdisloindex = (
        closest_to_zero_in_array(theta_ref)
    )
    ...
    for i_b in range(len(b_cases_all_burgers_direction_scaled)):
        ...
        mse_osc = np.mean(
            np.maximum(
                (f_osc_final_ref - predicted_phase) ** 2, floor_error**2
            )
        )
        mse_fit = np.mean(
            np.maximum(
                (f_fitosc_final_ref - predicted_phase) ** 2, floor_error**2
            )
        )
    ...
    for ii_scan, i_scan in enumerate(all_results_exp_scan_list):
        ...
        s_results_exp_xdisloval, s_results_exp_xdisloindex = (
            closest_to_zero_in_array(theta_ref)
        )
        ...
        for i_b in range(len(b_cases_all_burgers_direction_scaled)):
            ...
            mse_fit = np.mean(
                np.maximum(
                    (f_fitosc_exp - predicted_phase) ** 2, floor_error**2
                )
            )
...
def plot_phase_comparison_grid(
    ...
    for iii_scan, (i_scan, ax) in enumerate(zip(sorted_scan_keys, axs)):
        ...
        if all___phase_exp_all_ortho is not None:
            ...
            fit_phase = exp_data_phase - np.polyval(
                coefficients, exp_data_angle
            )
        ...
        if ref_coeffs is not None:
            ...
            ref_rms = np.sqrt(
                np.mean((center_angles(fit_phase) - ref_fit) ** 2)
            )
        ...
        ax.set_title(
            f"Best 3 Theoretical Fits for Scan {i_scan}", fontsize=fontsize
        )
...
# 🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀
def plot_simu_vs_exp_subplots_with_slope_table(
    ...
    for idx_plot, i in enumerate(included_indices):
        ...
        y_simu_filtered, pred_fit, _, f_linear_sim, coeffs_linear_sim = (
    ...
    fig.text(
        0.5, 0.04, "Polar Angle (rad)", ha="center", fontsize=fontsize + 1
    )
    fig.text(
        0.04,
        0.5,
...
def plot_simu_vs_exp_subplots_with_slope_table_v1(
    ...
    for idx_plot, i in enumerate(included_indices):
        ...
        if idx_plot == n_cols:
            ax.set_ylabel(
                "Phase Residual (rad)", fontsize=fontsize, labelpad=ylabelpad
            )
        ...
        if idx_plot == (n_rows_plot - 1) * n_cols + 1:
            ax.set_xlabel(
                "Polar Angle (rad)", fontsize=fontsize, labelpad=xlabelpad
            )
...
def plot_selected_simulations_with_theoretical_and_exp(
    ...
    for i_scan in range(len(selected_simulation_final)):
        ...
        label = (
            f"Sim {selected_simulation_final[i_scan]} (slope={simu_slope:.2f})"
        )
        h_sim = ax.plot(
            sim_angle, y_simu, "*", label=label, markersize=marker_size
        )[0]
...
# 🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀
def plot_phase_around_dislo_one_radius_new(
    amp,
    phase,
    selected_dislocation_data,
    r,
    ...
    if plot_debug:
        ...
        plt.plot(
            ...
            "o",
            label="r:%.1f" % r
            + r" $\Delta\phi$ %.2f rad" % (-y.min() + y.max()),
        )
        ax.legend(title="distance from\ndislocation center")
        # ax.legend()
        ax.set_xlabel("angle around dislocation (rad)", fontsize=15)
    ...
    while i < len(bad_slope):
        if bad_slope[i]:
            ...
            if end - start >= min_cluster:
                bad_mask[max(0, start - pad) : min(len(phase), end + pad)] = (
                    True
                )
        else:
            i += 1
...
def get_phase_simu_gvector(
    ...
    **🚀 Optimized for High-Resolution BCDI Dislocation Analysis! 🚀**
    ...
    def compute_phase_projection(data_disp, G):
        return -(
            G[0] * data_disp[0] + G[1] * data_disp[1] + G[2] * data_disp[2]
        )
    ...
    if plot_debug:
        ...
        if path_to_save is not None:
            plt.savefig(
                path_to_save + file_name + "Step1__0_raw_phase_no_op.png"
            )
        plt.show()
    if center_diffraction:
    ...
    if plot_debug:
        ...
        plt.show()
    simu_phase_ramp, _ = nan_to_zero(
        remove_phase_ramp_abd(zero_to_nan(simu_phase))
    )
    if plot_debug:
        plot_2D_slices_middle_one_array3D(
    ...
    strain_mask, strain_amp = compute_strain_map(
        obj, (1, 1, 1), nb_of_phase_to_test
    )
    ...
    gradient_modes_mask = (
        np.max(
            nan_to_zero(
                np.abs(
                    np.array(
                        get_displacement_gradient(_mask, voxel_size=(1, 1, 1))
                    )
                )
            ),
            axis=0,
    ...
    end_time = time.time()
    print(
        f"Processing took {np.round((end_time - start_time) / 60, 1)} minutes"
    )
    if plot_debug:
...
def fit_phase_correction_twodataset(
    ...
    def plot_phase_for_selected_points(
        ...
        num_rows = (num_points // num_cols) + (
            1 if num_points % num_cols != 0 else 0
        )
    ...
    for _ in range(num_trials):
        ...
        ransac = RANSACRegressor(
            min_samples=3, residual_threshold=0.05, max_trials=100
        )
        ransac.fit(
            A[:, :3], Delta_Phi
        )  # Fit only x, y, z (exclude bias term for now)
    ...
    if plot_debug:
        ...
        ax.plot(
            x_range, simu_values, "o-", color="blue", label="Simulated Phase"
        )
        ax.plot(
            x_range,
            theo_values,
...
def plot_filtered_phase_analysis(
    ...
    # Phase + angle (shifted)
    ax2.plot(
        angle_ring_3d, phase_ring_3d + sign * angle_ring_3d, "*", alpha=0.4
    )
...
# 🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀
def create_circular_mask_from_cylindrical_v1(
    R, Theta, Z_axis, r, dr, dz, z0=0.0
):
    """
...
def modify_shaperealspace_basedon_reciprocalspace(obj, shape_data_init):
    # Compute the 3D Fourier transform
    reciprocal_space_object = np.fft.ifftshift(
        np.fft.fftn(np.fft.fftshift(obj))
    )
...
def compute_cylindrical_coords_fromnonorthogonalspace(
    a, b, c, centroid, direction
):
    # 1. Shift coordinates
    X = a - centroid[0]
    Y = b - centroid[1]
...
def step4_process_phase_ring_orthogonal(
    ...
    for i_radius in list_radius:
        ...
        # ]
        angle_ring_3d = all_data_Step3_raw_and_ortho_dict[
            f"angle_ring_3d_{i_radius}"
        ]
        phase_ring_3d = all_data_Step3_raw_and_ortho_dict[
            f"phase_ring_3d_{i_radius}"
        ]
        vect_x_lab = all_data_Step3_raw_and_ortho_dict[
            f"vect_x_lab_{i_radius}"
        ]
        vect_y_lab = all_data_Step3_raw_and_ortho_dict[
            f"vect_y_lab_{i_radius}"
        ]
        vect_z_lab = all_data_Step3_raw_and_ortho_dict[
            f"vect_z_lab_{i_radius}"
        ]
        ...
        disp_vect = np.stack(
            [vect_x_lab, vect_y_lab, vect_z_lab], axis=-1
        ).astype(np.float32)
        ...
        *_, disp_vect_ring_sorted, disp_vect_final = (
            dislo_process_phase_ring_ortho(
                angle_ring_3d,
                phase_ring_3d,
                disp_vect,
        ...
        results_phase_experiment_ortho[str(i_radius)]["data_processed"][
            scan
        ] = {
            "angle": angle_final,
            "phase": phase_final,
