...
# from cdi_dislo.utils.utils import mask_clusters
# import os
# import h5py
# import glob
# import pandas as pd
...
def get_prediction_from_theo_sapphire_lattice(T_celsius, plot=False):
    ...
    print(
        f"Lattice parameter at 27°C: {calculate_lattice_param(base_param, T_target):.6f}"
    )
    ...
    print(
        f"Value of lattice parameter at {T_celsius:.2f}°C fit poly 4 degree: {predict_at_T_poly:.6f}"
    )
    ...
    if plot:
        ...
        plt.plot(
            ...
            def_theo_sapphire,
            "+",
            label="Adjusted Theoretical Sapphire",
        )
        ...
        plt.plot(
            ...
            curve_fit_theo_par,
            "g--",
            label=f"Polynomial Fit | R2={r2_score(def_theo_sapphire, poly_function(T_data_sapphire)):.4f}",
        )
...
def GET_theta_lambda_energy_latticeconstant_reference(
    ...
    lambda_ = (
        2.0
        * get_prediction_from_theo_sapphire_lattice(
            int(temp_experimental[temp_rt]), plot=False
        )[1]
        / 6.0
    ) * np.sin(theta[temp_rt])
    energy = lam2en(lambda_) / 1000

    C_ = 3.0 * lambda_ / np.sin(theta)
...
#####################################################################################################################
def calculate_epsilon_and_prediction(
    T_data_exp,
    results_sapphire_d_spacing,
    fit_order=2,
    plot=True,
    save_plot=None,
    a0=12.991,
    ...
    def get_temperatureprediction_fromdeformation_and_fitparametre(
        curve_fit, popt, fit_order
    ):
        if fit_order == "exp":
            T_predict = ((1 / popt[1]) * np.log(popt[0] / curve_fit)) + popt[2]
            ...
            if fit_order == 1:
                ...
                for i in curve_fit:
                    result = [
                        popt[ii] - float(i * float(ii == fit_order))
    ...
    if fit_order == "exp":
        ...
        def func_def(T_, a, b, c):  # type: ignore
        ...
        if fit_order == 1:
            ...
            def func_def(T_, a, b):  # pyright: ignore[reportRedeclaration]
                return a * T_ + b

            fit_degree = "1st"
        elif fit_order == 2:

            def func_def(T_, a, b, c):  # pyright: ignore[reportRedeclaration]
                return a * T_**2 + b * T_ + c

            fit_degree = "2nd"
        elif fit_order == 3:

            def func_def(T_, a, b, c, d):  # pyright: ignore[reportRedeclaration]
                return a * T_**3 + b * T_**2 + c * T_ + d

            fit_degree = "3rd"
        elif fit_order == 4:

            def func_def(T_, a, b, c, d, e):  # pyright: ignore[reportRedeclaration]
    ...
    popt_def, pcov_def = curve_fit(
        func_def_theosapphire, T_data_sapphire, def_theo_sapphire, maxfev=10**5
    )
    ...
    popt, pcov = curve_fit(
        func_def, T_theo_predict, results_sapphire_d_spacing, maxfev=10**5
    )
    ...
    if plot:
        ...
        if fit_order == 1:
            text_annote_fit_res = (
                r"$a_{\mathrm{fit(exp)}} = %.2f\,T \times 10^{%0.0f} + %.3f$"
                % (
                    extract_coefficient_and_exponent(popt[0])[0],
                    extract_coefficient_and_exponent(popt[0])[1],
                    popt[1],
                )
            )
            text_annote_fit_res_epsilon = (
                r"$\epsilon_{\mathrm{fit(exp)}} = %.2f\,T \times 10^{%0.0f} + %.3f\times 10^{%0.0f}$"
                % (
                    extract_coefficient_and_exponent(popt[0] / a0)[0],
                    extract_coefficient_and_exponent(popt[0] / a0)[1],
                    extract_coefficient_and_exponent((popt[1] - a0) / a0)[0],
                    extract_coefficient_and_exponent((popt[1] - a0) / a0)[1],
                )
            )
        if fit_order == 2:
            text_annote_fit_res = (
                r"$a_{\mathrm{fit(exp)}} = %.2f\,T^2 \times 10^{%0.0f} + %.2f\,T \times 10^{%0.0f} + %.3f$"
                % (
                    extract_coefficient_and_exponent(popt[0])[0],
                    extract_coefficient_and_exponent(popt[0])[1],
                    extract_coefficient_and_exponent(popt[1])[0],
                    extract_coefficient_and_exponent(popt[1])[1],
                    popt[2],
                )
            )
            text_annote_fit_res_epsilon = (
                r"$\epsilon_{\mathrm{fit(exp)}} = %.2f\,T^2 \times 10^{%0.0f} + %.2f\,T \times 10^{%0.0f} + %.3f$"
                % (
                    extract_coefficient_and_exponent(popt[0] / a0)[0],
                    extract_coefficient_and_exponent(popt[0] / a0)[1],
        ...
        if fit_order == 3:
            text_annote_fit_res = (
                r"$a_{\mathrm{(fit exp)}} = %.2f\,T^3 \times 10^{%0.0f} +%.2f\,T^2 \times 10^{%0.0f} + %.2f\,T \times 10^{%0.0f} + %.3f$"
                % (
                    extract_coefficient_and_exponent(popt[0])[0],
                    extract_coefficient_and_exponent(popt[0])[1],
                    extract_coefficient_and_exponent(popt[1])[0],
                    extract_coefficient_and_exponent(popt[1])[1],
                    extract_coefficient_and_exponent(popt[2])[0],
                    extract_coefficient_and_exponent(popt[2])[1],
                    popt[3],
                )
            )
            text_annote_fit_res_epsilon = (
                r"$\epsilon_{\mathrm{fit(exp)}} = %.2f\,T^3 \times 10^{%0.0f} +%.2f\,T^2 \times 10^{%0.0f} + %.2f\,T \times 10^{%0.0f} + %.3f$"
                % (
                    extract_coefficient_and_exponent(popt[0] / a0)[0],
                    extract_coefficient_and_exponent(popt[0] / a0)[1],
                    extract_coefficient_and_exponent(popt[1] / a0)[0],
        ...
        if fit_order == 4:
            text_annote_fit_res = (
                r"$a_{\mathrm{fit(exp)}} = %.2f\,T^4 \times 10^{%0.0f} + %.2f\,T^3 \times 10^{%0.0f} + %.2f\,T^2 \times 10^{%0.0f} + %.2f\,T \times 10^{%0.0f} + %.3f$"
                % (
                    extract_coefficient_and_exponent(popt[0])[0],
                    extract_coefficient_and_exponent(popt[0])[1],
                    extract_coefficient_and_exponent(popt[1])[0],
                    ...
                    extract_coefficient_and_exponent(popt[2])[0],
                    extract_coefficient_and_exponent(popt[2])[1],
                    extract_coefficient_and_exponent(popt[3])[0],
                    extract_coefficient_and_exponent(popt[3])[1],
                    popt[4],
                )
            )
            text_annote_fit_res_epsilon = (
                r"$\epsilon_{\mathrm{fit(exp)}} = %.2f\,T^4 \times 10^{%0.0f} + %.2f\,T^3 \times 10^{%0.0f} + %.2f\,T^2 \times 10^{%0.0f} + %.2f\,T \times 10^{%0.0f} + %.3f$"
                % (
                    extract_coefficient_and_exponent(popt[0] / a0)[0],
                    extract_coefficient_and_exponent(popt[0] / a0)[1],
                    extract_coefficient_and_exponent(popt[1] / a0)[0],
        ...
        if fit_order == 5:
            text_annote_fit_res = (
                r"$a_{\mathrm{fit(exp)}} = %.2f\,T^5 \times 10^{%0.0f} + %.2f\,T^4 \times 10^{%0.0f} + %.2f\,T^3 \times 10^{%0.0f} + %.2f\,T^2 \times 10^{%0.0f} + %.2f\,T \times 10^{%0.0f} + %.3f$"
                % (
                    extract_coefficient_and_exponent(popt[0])[0],
                    extract_coefficient_and_exponent(popt[0])[1],
                    extract_coefficient_and_exponent(popt[1])[0],
                    ...
                    extract_coefficient_and_exponent(popt[3])[0],
                    extract_coefficient_and_exponent(popt[3])[1],
                    extract_coefficient_and_exponent(popt[4])[0],
                    extract_coefficient_and_exponent(popt[4])[1],
                    popt[5],
                )
            )
            text_annote_fit_res_epsilon = (
                r"$\epsilon_{\mathrm{fit(exp)}} = %.2f\,T^5 \times 10^{%0.0f} + %.2f\,T^4 \times 10^{%0.0f} + %.2f\,T^3 \times 10^{%0.0f} + %.2f\,T^2 \times 10^{%0.0f} + %.2f\,T \times 10^{%0.0f} + %.3f$"
                % (
                    extract_coefficient_and_exponent(popt[0] / a0)[0],
                    extract_coefficient_and_exponent(popt[0] / a0)[1],
                    extract_coefficient_and_exponent(popt[1] / a0)[0],
        ...
        if fit_order == 6:
            text_annote_fit_res = (
                r"$a_{\mathrm{fit(exp)}} = %.2f\,T^6 \times 10^{%0.0f} + %.2f\,T^5 \times 10^{%0.0f} + %.2f\,T^4 \times 10^{%0.0f} + %.2f\,T^3 \times 10^{%0.0f} + %.2f\,T^2 \times 10^{%0.0f} + %.2f\,T \times 10^{%0.0f} + %.3f$"
                % (
                    extract_coefficient_and_exponent(popt[0])[0],
                    extract_coefficient_and_exponent(popt[0])[1],
                    extract_coefficient_and_exponent(popt[1])[0],
                    ...
                    extract_coefficient_and_exponent(popt[4])[0],
                    extract_coefficient_and_exponent(popt[4])[1],
                    extract_coefficient_and_exponent(popt[5])[0],
                    extract_coefficient_and_exponent(popt[5])[1],
                    popt[6],
                )
            )
            text_annote_fit_res_epsilon = (
                r"$\epsilon_{\mathrm{fit(exp)}} = %.2f\,T^6 \times 10^{%0.0f} + %.2f\,T^5 \times 10^{%0.0f} + %.2f\,T^4 \times 10^{%0.0f} + %.2f\,T^3 \times 10^{%0.0f} + %.2f\,T^2 \times 10^{%0.0f} + %.2f\,T \times 10^{%0.0f} + %.3f$"
                % (
                    extract_coefficient_and_exponent(popt[0] / a0)[0],
                    extract_coefficient_and_exponent(popt[0] / a0)[1],
                    extract_coefficient_and_exponent(popt[1] / a0)[0],
        ...
        if fit_order == "exp":
            text_annote_fit_res = (
                r"$a_{\mathrm{fit(exp)}} = %.2f \times e^{%.2f(T - %.2f)}$"
                % (popt[0], popt[1], popt[2])
            )
            text_annote_fit_res_epsilon = (
                r"$\epsilon_{\mathrm{fit(exp)}} = %.2f \times e^{%.2f(T - %.2f)}$"
        ...
        plt.text(
            0.02,
            0.95,
            text_annote_fit_res,  # pyright: ignore[reportPossiblyUnboundVariable]
        ...
        plt.plot(
            ...
            "o",
            label="epsilon data exp",
        )
        plt.plot(
            T_data_plot,
            epsilon_fit_theo_par,
            "r-",
            alpha=0.5,
            label="epsilon fit theo",
        )
        plt.plot(
            T_data_sapphire,
            epsilon_theo_sapphire,
            "r*",
            alpha=0.5,
            label="epsilon data theo",
        )
        ...
        plt.text(
            0.02,
            0.95,
            text_annote_fit_res_epsilon,  # pyright: ignore[reportPossiblyUnboundVariable]
            transform=plt.gca().transAxes,
            bbox=dict(facecolor="blue", alpha=0.9),
            fontsize=12,
            color="white",
        ...
        plt.tight_layout()
        plt.suptitle(
            f"Temperature calibration for experiment {experiment} ({fit_degree} degree polynomial fit)",
            y=1.01,
        )
        if save_plot:
...
def calculate_epsilon_and_prediction_2nd_degree(
    T_data_exp,
    results_sapphire_d_spacing,
    plot=True,
    save_plot=None,
    ...
    popt, pcov = curve_fit(
        func_def, T_data_exp, results_sapphire_d_spacing, maxfev=10**5
    )
    ...
    Avril_2023_epsilon_curve_fit_exp_par = np.round(
        100 * ((Avril_2023_curve_fit_exp_par - a0) / a0), 2
    )
    epsilon_results_sapphire_d_spacing = np.round(
        100 * ((results_sapphire_d_spacing - a0) / a0), 2
    )
    ...
    epsilon_theo_sapphire = np.round(
        100 * ((Avril_2023_def_theo_sapphire - a0) / a0), 2
    )
    ...
    epsilon_fit_theo_par = np.round(
        100 * ((Avril_2023_curve_fit_theo_par - a0) / a0), 2
    )
    ...
    if plot:
        ...
        plt.subplot(3, 1, 1)
        plt.plot(
            Avril_2023_T_data_plot,
            Avril_2023_curve_fit_exp_par,
            "-",
            label="fit exp",
        )
        plt.plot(T_data_exp, results_sapphire_d_spacing, "o", label="data exp")
        ...
        plt.plot(
            Avril_2023_T_data_plot,
            Avril_2023_epsilon_curve_fit_exp_par,
            label="epsilon fit exp",
        )
        plt.plot(
            T_data_exp,
            epsilon_results_sapphire_d_spacing,
        ...
        plt.tight_layout()
        plt.suptitle(
            f"Temperature calibration for experiment {experiment} (2nd-degree polynomial fit)",
            y=1.02,
        )
    ...
    return (
        Avril_2023_T_data_plot,
        Avril_2023_T_exp_predict,
        Avril_2023_T_theo_predict,
        Avril_2023_curve_fit_exp_par,
        Avril_2023_curve_fit_theo_par,
        Avril_2023_def_theo_sapphire,
        Avril_2023_epsilon_curve_fit_exp_par,
        epsilon_results_sapphire_d_spacing,
        epsilon_fit_theo_par,
        epsilon_theo_sapphire,
...
#####################################################################################################################
def plot_temperature_and_lattice(
    nov_2022_best_T_exp,
    nov_2022_best_T_theo,
    nov_2022_best_delta_temp,
    nov_2022_C_,
    save_plot=None,
):
    """
...
def process_scan_data_COM_CRISTAL_2022(
    ...
    for i_part in particle_list_B12S1P1:
        ...
        list_scan_part_B12S1P1 = {
            **list_scan_part_B12S1P1,
            i_part: np.array(list_s).astype(str),
        }
    ...
    for i in range(0, len(scan_list_det)):
        ...
        check_part = [
            scan_list_det[i] in list_scan_part_B12S1P1[i_part]
            for i_part in list_scan_part_B12S1P1
        ]
        try:
            scan_exist = True
        ...
        if scan_exist:
            ...
            delta, gamma, omega = (
                ...
                np.array(f[key_gamma][()]).mean(),
                np.array(f[key_omega][()]),
            ...
            if scan_list_det[i] in np.array(
                [
                    188,
                    491,
                    495,
            ...
            # Plot original data
            plot_summary_difraction(
                zero_to_nan(data_original),
                com_,
                max_coords,
                path_save=path_results_saveplots + "original/",
                fig_title=fig_title,
                fig_save_=f"{fig_title}_original.png",
                f_s=f_s,
                vmin=vmin,
                vmax=vmax,
            )
...
def process_scan_data_COM_id1_avril_2023(
    ...
    dataset_name = [
        "B12-S1P1_0001.h5",
        "B12-S1P1_BCDI.h5",
        "B12-S1P1_B12_B1P1_BCDI_1_0002.h5",
        "B12-S1P1_B12_B1P1_BCDI_3.h5",
    ]
    ...
    for i_sample, location in enumerate(data_location):
        ...
        # all_scans_not_filtrerd_folders = np.array(
        #     [f"{int(item):04d}" for item in all_scans_not_filtrerd_INT]
        # )
        # scans_noteta = np.array(
        #     [
        #         f"{int(i_scan[:-2]):04d}"
        #         for i_scan in all_scans_not_filtrerd
        #         if "eta" not in f[i_scan]["title"][()].decode()
        #     ]
        # )  # pyright: ignore[reportAttributeAccessIssue]
        scans_eta = np.array(
            [
                i_scan
                for i_scan in all_scans_not_filtrerd
                if ("eta" in f[i_scan]["title"][()].decode())
            ]
        )  # type: ignore
        scans_eta_1d_scan_nmpoints = np.array(
            [
                i_scan[:-2]
                for i_scan in scans_eta
                if (
                    (float(f[i_scan]["title"][()].decode()[-2:]) >= 1.0)
                    and (float(f[i_scan]["title"][()].decode()[-5:-2]) >= 200)
                )
            ]
        )  # pyright: ignore[reportAttributeAccessIssue]
        ...
        for i in range(len(scan_dict_all[i_dataset])):
            ...
            if scan_exist:
                ...
                if filename == "77":
                    ...
                    check_part = [
                        filename in list_scan_part_B12S1P1[i_part]
                        for i_part in list_scan_part_B12S1P1
                    ]
                    indice_part = np.where(check_part)[0][0]
                ...
                plt.savefig(
                    os.path.join(
                        path_results_saveplots,
                        f"{fig_title}_3D_projections.png",
                    )
                )
                plt.close()
...
def process_bcdi_data_B12S1P1_id1_Jan_2024(
    ...
    particles_scans_TS = pd.read_csv(
    ...
    particles_scans_P1 = pd.read_csv(
        file_name, delimiter=";", encoding="unicode_escape"
    )
    ...
    for i in range(0, len(scan_list_det)):
        ...
        check_part = [
            scan_list_det[i] in list_scan_part_P1[i_part]
            for i_part in list_scan_part_P1
        ]
        try:
            scan_exist = True
            indice_part = np.where(check_part)[0][0]
...
def process_scan_data_COM_id1_june_2024(
    ...
    def process_data(data_original, part_name, scan, len__):
        ...
        if ((part_name == "II-D8") and (int(scan) >= 87)) or (
            (part_name == "II-G10") and (int(scan) > 68)
        ):  # or ((part_name=="VII-A7") and ( int(scan)<= 132)):
            masked_data, data_original_copy, dilated_mask = mask_clusters(
                data_original,
                threshold_factor=0.3,
            ...
            # max_ = [max_x, max_y, max_z]
            half_w = 80
            data_original_copy[: max_x - half_w, :, :] = 0
            data_original_copy[:, : max_y - half_w, :] = 0
            data_original_copy[:, :, : max_z - half_w] = 0
            data_original_copy[max_x + half_w :, :, :] = 0
            data_original_copy[:, max_y + half_w :, :] = 0
            data_original_copy[:, :, max_z + half_w :] = 0
...
def process_scan_id1_june_2024(
    ...
    def apply_mask(data, threshold, iterations, second_cluster=False):
        ...
        masked_data, data_copy, _ = mask_clusters(
            data,
    ...
    if part_name == "II-D8" and scan == 11:
    ...
    elif (part_name == "II-D8" and scan >= 87) or (
        part_name == "II-G10" and scan <= 408
    ):
        data_copy = apply_mask(data_original, 0.3, 50)

    elif (part_name == "II-G10" and scan == 67) or (
        part_name == "VII-A7" and scan == 88
    ):
        ...
        data_copy[:, :, max_z + half_w :] = 0
    max_coords = get_max_coords(data_copy)
    com = [int(np.round(i)) for i in C_O_M(data_copy)]
    print(com)
    ...
    if plot_original:
        plot_summary_difraction(
            ...
            com,
            max_coords,
            path_save=plot_original + "original",
            fig_title=fig_title,
            fig_save_=fig_save_,
            f_s=20,
            vmin=0,
...
def optimize_energy_value_and_calibrate_temperature(
    original_energy,
    results_sapphire,
    Temp_experimentale,
    a_at_20TC=12.991,
    ...
    for energy in energy_values:
        try:
            ...
            new_d_spacing = get_lattice_parametre(
                ...
                [
                    results_sapphire[:, 12].astype(float),
                    results_sapphire[:, 13].astype(float),
                ],
            ...
            if len(new_d_spacing) != len(Temp_experimentale):
                print(
                    f"Mismatch in lengths: new_d_spacing ({len(new_d_spacing)}) and Temp_experimentale ({len(Temp_experimentale)})"
                )
                continue
            ...
            if minimize_method == "sum":
                ...
                if specific_index is None or specific_index >= len(delta_temp):
                    raise ValueError(
                        "Invalid specific_index for minimize_method 'index'"
                    )
    ...
    if plot:
        ...
        plt.plot(
            best_T_exp,
            thermal_expansion(best_T_exp, *params),
            "-",
            label="Fitted curve",
        )
        plt.xlabel(r"$T_{(°C)}$", fontsize=14)
        ...
        plt.tight_layout()
        plt.suptitle(
            f"Energy Optimization Results (Best Energy: {best_energy:.3f} keV, Metric: {min_metric:.5f})",
            y=1.02,
        )
