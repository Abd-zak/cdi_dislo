...
1. Fixed function name in the docstring example:
...
4. Optimized `try-except` blocks:
   - Combined multiple `try-except` blocks for plotting into a single block.

5. Improved shell command handling (`os.system`):
...
#####################################################################################################################
#####################################################################################################################
def selection_of_reco_in_path(
    path_to_reco: str = "/home/abdelrahman/cristal_Nov_2022/",
    particle: str = "B8",
    scan: str = "S400",
    ...
    rc["font.size"] = 14
    rc["xtick.labelsize"] = 14  # type: ignore
    rc["ytick.labelsize"] = 14
    rc["figure.autolayout"] = True
    plt.rcParams["text.usetex"] = False  # type: ignore
    ...
    if check_array_empty(files):
        print(
            f"No .CXI files found in {wdir}. Please check if prefix_to_reco_2 parametre well defined (path to .CXI reco files)"
        )
        return
    wdir_save_reco_sel = (
        path_to_reco
    ...
    llkf_run = [
        IfStringRepresentsFloat(
            i_run[i_run.find("_LLKf") + len("_LLKf") : i_run.find("_LLK0")]
        )
        for i_run in files
    ]
    print(llkf_run)
    ...
    def data_read_selection(files, multiply_by_mask):
        ...
        """
        start = time.time()
        data_allrun, rho_data, phi_data = [], [], []
        # size = len(files)
        pad_add = 20
        data_allrun = np.array(
            [
                np.pad(
                    np.array(
                        load_reco_from_cxinpz(
        ...
        rho_data = np.array(
            [
                crop_3darray_pos(
                    rho_data[i],
                    methods=com_int[i],
                    output_shape=(half_w, half_w, half_w),
                )
    ...
    if selection_method != "filter_by_llkf":
        ...
        if not load_data:
            ...
            np.savez(
                wdir_save + "all_part_scan_results.npz",
                **{
                    "rho_data": rho_data,
                    "phi_data": phi_data,
        ...
        print(str(int(((end - start) / 60) * 10) / 10) + "min")
        np.savez(
            wdir_save + "all_part_scan_results_selection.npz",
            **{"STD_rho": results_STD, "list_llkf_allscan": list_llkf},
        )
        f = np.load(
            wdir_save + "all_part_scan_results_selection.npz",
            allow_pickle=True,
    ...
    if selection_method == "filter_by_llkf":
        ...
        while True:
            ...
            print(
                ...
                + scan
                + "*********"
                + str(len(select_nb_pixel))
                + "*********"
                + str(np.round(MIN_sumpixel, 4))
            )
            ...
            print(
                ...
                + scan
                + "*********"
                + str(len(trigger_STDRHO))
                + "*********"
                + str(np.round(max_stdrho, 4))
            )
            print(
                "combine the selections based on std density, the LLKf and the # in the support "
            )
            trigger___ = []
            for i_run in trigger_llk:
                if (
                    (i_run in select_nb_pixel)
                    and (i_run in trigger_STDRHO)
                    and (i_run in trigger_llk)  # and
                ):
    ...
    try:
        plt.figure(1, figsize=(9, 5))
        a = np.max(RESULTS_LLKf[RESULTS_LLKf != 10.0])  # type: ignore
        y_sel = np.asarray(RESULTS_stdrho)[
            np.hstack(trigger_LLK_pixelNB_mtm_stdrho)
        ]  # type: ignore
        x_sel = np.asarray(RESULTS_LLKf)[
            np.hstack(trigger_LLK_pixelNB_mtm_stdrho)
        ]  # type: ignore
        p = plt.plot(
            RESULTS_LLKf, RESULTS_stdrho, "*", linewidth=2, label="all runs"
        )  # type: ignore
        plt.plot(
            [RESULTS_LLKf.min(), RESULTS_LLKf.max()],
            [max_stdrho, max_stdrho],
            c=p[0].get_color(),
        )  # type: ignore
        plt.plot(
            [max_llkf, max_llkf],
            [RESULTS_stdrho.min(), RESULTS_stdrho.max()],
            c=p[0].get_color(),
        )  # type: ignore
        p = plt.plot(x_sel, y_sel, ">", linewidth=2, label="selected")
    ...
    if create_mode:
        ...
        if ramp_mode:
            ...
            os.system(
                f"cd {wdir_save_reco_sel}; pynx-cdi-analysis *cxi modes=1 modes_output={mode_output_file_name}"
            )
    if save_animation:
