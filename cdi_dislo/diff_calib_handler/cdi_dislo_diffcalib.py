...
# Standard libraries
import os  , glob, math, h5py ,importlib
import numpy as np
import pandas as pd
...
#####################################################################################################################
def calculate_epsilon_and_prediction(T_data_exp, 
                                     results_sapphire_d_spacing,
                                       fit_order=2,
                                         plot=True,save_plot=None,a0=12.991,
                                         experiment=""):
    import numpy as np
    import matplotlib.pyplot as plt
    from scipy.optimize import curve_fit
    ...
    T_data_sapphire   = np.array([  0. ,  50. , 100. , 150. , 200. , 250. , 300., 350. , 400. , 450. , 500. , 550.  , 600.  , 650.  , 700.  , 750.  , 800  , 900  ,1000  ,1100   , 1200    , 1300   , 1400    ,1500    , 1600   , 1700    ,   1800  , 1900   , 2000  
                               ]) - 273
    def_theo_sapphire = np.array([-5.33, -5.33, -5.21, -4.56, -3.20, -1.16, 1.44, 4.51 , 7.94 , 11.65, 15.58, 19.69 , 23.94 , 28.31 , 32.77 , 37.31 , 41.92, 51.29, 60.82, 70.48 , 80.09   , 89.76  , 99.47   ,  109.2 , 119.0  , 128.7   , 138.6   , 148.4  , 158.2]) * 10 ** -4
    ...
    T_exp_predict = np.array(T_data_exp)#get_temperatureprediction_fromdeformation_and_fitparametre (results_sapphire_d_spacing,popt) 
    ...
    if plot:
        ...
        plt.text(0.02, 0.95,text_annote_fit_res , # pyright: ignore[reportPossiblyUnboundVariable]
        ...
        plt.text(0.02, 0.95,text_annote_fit_res_epsilon , # pyright: ignore[reportPossiblyUnboundVariable]
            transform=plt.gca().transAxes,
            bbox=dict(facecolor='blue', alpha=0.9),
            fontsize=12, color='white')
        plt.legend(ncol=2,loc=4, fontsize=12)
...
def calculate_epsilon_and_prediction_2nd_degree(T_data_exp,
                                                results_sapphire_d_spacing, 
                                                plot=True, 
                                                save_plot=None,
                                                a0=12.991, 
...
def process_scan_data_COM_CRISTAL_2022(
    ...
    for i in range(0, len(scan_list_det)):
        ...
        if scan_exist:
            ...
            key_path = [pref_key + i for i in (h5py.File(filename)[pref_key]).keys() if (len(h5py.File(filename)[pref_key][i].shape) == 3)][0] # pyright: ignore[reportIndexIssue, reportAttributeAccessIssue]
            key_pos = scan_list_det_datah5[i] + '/CRISTAL/Diffractometer/'
            key_gamma = [key_pos + i for i in (h5py.File(filename)[key_pos]).keys() if ('gamma' in i)][0] + '/position' # pyright: ignore[reportAttributeAccessIssue]
            key_delta = [key_pos + i for i in (h5py.File(filename)[key_pos]).keys() if ('delta' in i)][0] + '/position' # pyright: ignore[reportAttributeAccessIssue]
            key_omega = [pref_key + i for i in (h5py.File(filename)[pref_key]).keys() if ('actuator_1_1' in i)][0] # pyright: ignore[reportAttributeAccessIssue]
            ...
            delta, gamma, omega = np.array(f[key_delta][()]).mean(), np.array(f[key_gamma][()]).mean(), np.array(f[key_omega][()]) # pyright: ignore[reportIndexIssue]
            data_original = f[key_path][()] # pyright: ignore[reportIndexIssue]
            data_original = np.array([data_original[i] * (1 - mask) for i in range(len(data_original))]) # pyright: ignore[reportArgumentType, reportIndexIssue]
...
def process_scan_data_COM_id1_avril_2023(
    ...
    for i_sample, location in enumerate(data_location):
        ...
        scans_noteta = np.array([f'{int(i_scan[:-2]):04d}' for i_scan in all_scans_not_filtrerd if 'eta' not in f[i_scan]['title'][()].decode()]) # pyright: ignore[reportAttributeAccessIssue]
        scans_eta = np.array([i_scan for i_scan in all_scans_not_filtrerd if ('eta' in f[i_scan]['title'][()].decode())]) # type: ignore
        scans_eta_1d_scan_nmpoints = np.array([i_scan[:-2] for i_scan in scans_eta if ((float(f[i_scan]['title'][()].decode()[-2:]) >= 1.) and (float(f[i_scan]['title'][()].decode()[-5:-2]) >= 200))]) # pyright: ignore[reportAttributeAccessIssue]
...
def process_bcdi_data_B12S1P1_id1_Jan_2024(
    ...
    for i in range(0, len(scan_list_det)):
        ...
        if scan_exist:
            ...
            delta, eta, nu = f[key_pos + 'delta'][()], f[key_pos + 'eta'][()], f[key_pos + 'nu'][()] # pyright: ignore[reportIndexIssue]
            ...
            # Plot cropped data
            plot_summary_difraction(zero_to_nan(data_cropped), com_crop, max_crop,path_save=path_results_saveplots+"cropped/", # pyright: ignore[reportArgumentType]
                                    fig_title=fig_title, fig_save_=f"{fig_title}_crop.png",f_s=f_s, vmin=vmin, vmax=vmax)

            ressss = [part_name, filename, temp_scan, nu.mean(), delta.mean(),eta[int(com_[0])], eta.mean(), com_[0], com_[1], com_[2], # type: ignore
                      data_original.max(), data_original.sum(), 363.908464, 144.5] # pyright: ignore[reportAttributeAccessIssue]
...
def process_scan_data_COM_id1_june_2024(
    ...
    def process_data(data_original, part_name, scan, len__):
        ...
        if ((part_name=="II-D8") and ( int(scan)>= 87)) or ((part_name=="II-G10") and ( int(scan)>68)) :#or ((part_name=="VII-A7") and ( int(scan)<= 132)):
            ...
            data_original_copy[:, max_y+half_w:, :] = 0 ;    data_original_copy[:, :, max_z+half_w:] = 0 ;
