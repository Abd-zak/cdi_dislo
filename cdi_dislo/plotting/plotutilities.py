...
def visualize_3d_data(
    ...
    grid.dimensions = tuple(
        np.array(data_array.shape)
    )  # Ensure correct point dimensions
...
def plot_interactive_slices(data, default_axis="z", clim=None, cmap="Viridis"):
    ...
    interact(
        ...
        slice_index=widgets.IntSlider(
            min=0,
            max=data.shape[
                2 if default_axis == "z" else 1 if default_axis == "y" else 0
            ]
            - 1,
            step=1,
            value=0,
...
def plot_summary_difraction(
    ...
    ax[0].scatter(
        ...
        alpha=0.5,
        color="red",
        label=f" COM: {com_[0]},{com_[1]},{com_[2]}",
    )
    ax[0].scatter(
        max_[2],
        max_[1],
        ...
        alpha=0.5,
        color="black",
        label=f" MAX: {max_[0]},{max_[1]},{max_[2]}",
    )
    ...
    proj_xy = np.log(
        np.nanmax(data_original, axis=0) + eps
    )  # (Y, X), label Z on x in your convention
    ...
    fig.tight_layout()
    plt.savefig(
        path_save + "_xproj_" + fig_save_, dpi=300, bbox_inches="tight"
    )  # type: ignore
    plt.show()
...
def plot_mechanical_properties(
    ...
    for ax, title, xlabel, ylabel, x, y in zip(
        axes, titles, x_labels, y_labels, x_data, y_data
    ):
        ...
        if plot_fit:
            ...
            if r_squared > r_squared_threshold:
                ...
                print(
                    f"R-squared ({r_squared:.3f}) below threshold for {title}. Fit not plotted."
                )
    ...
    if plot_fit:
        ...
        for i, (x, y, title) in enumerate(zip(x_data, y_data, titles), 1):
            ...
            if r_squared <= r_squared_threshold:
                print("R-squared below threshold. Fit not plotted.")
...
def get_color_list(nb):
    ...
    dark_colors = [
        color
        for color in colors_comb
        if mcolors.to_rgba(color)[0] * 0.299
        + mcolors.to_rgba(color)[1] * 0.587
        + mcolors.to_rgba(color)[2] * 0.114
    ...
    for i, j in combinations(range(len(rgb_colors)), 2):
        color_diffs[i, j] = np.linalg.norm(
            np.array(rgb_colors[i]) - np.array(rgb_colors[j])
        )
...
def get_x_y_of_drops_plastic(x, y, plot_debug=False):
    ...
    for i in linspace(x_min + 1, x_min + 50, 50):
        ...
        with warnings.catch_warnings():
            ...
            coef = np.polyfit(
                x_new,
                y_new + random.normal(size=len(y_new), scale=0.250, loc=0),
                1,
            )
    ...
    if plot_debug:
        ...
        for i in range(len(rising_edges) - 1):
            ...
            plt.plot(
                ...
                new_y_phase1[start_index:end_index],
                ".",
                markersize=1.0,
                label=i,
            )
...
def plot_selcted_runs_y(
    index_best_run_scans,
    data_scans,
    data_allscans_runs,
    data_allscans_LLK,
    param,
    ...
    for i_scan in index_best_run_scans.keys():
        ...
        for i in index_best_run_scans[i_scan]:
            ...
            if param == "phi":
                ...
                im = ax.imshow(
                    mean_y_run(data_scans[i_scan][i]),
                    cmap="jet",
                    vmin=0,
                    vmax=6,
                )
            ...
            ax.set_title(
                "Run:"
                + str(int(data_allscans_runs[i_scan][i]))
                + "/ LLK:"
                + str(int(data_allscans_LLK[i_scan][i] * 10000) / 10000)
                + "/ mean "
                + param
                + " :"
                + str(int(mean_data(data_scans[i_scan][i]) * 10000) / 10000)
                + "/ $std_{\\rho}$ :"
                + str(int(std_data(data_scans[i_scan][i]) * 10000) / 10000),
                fontsize=font_size,
                pad=40,
            )
...
def plot_data_disp(data_, lim):
    ...
    fig, axs = plt.subplots(
        nrows=n_sbplots_row,
        ncols=n_sbplots_cols,
        figsize=(4 * n_sbplots_cols, 3 * n_sbplots_row),
    )
    for i_frame in range(6):
        ...
        ax.set_ylabel("X", labelpad=5)
        color_bar.set_label(
            "$\\epsilon_{}$".format(i_frame + 1), labelpad=5, fontsize=18
        )
        plt.grid(None)
        ax.invert_yaxis()
        ax.set_title("Mean Z ")
        plt.axis("tight")
    fig.suptitle(
        "Strain of platinium nanoparticle",
        fontweight="bold",
        horizontalalignment="center",
        y=1,
    )
...
def plot_data_disp_projection(data_, plan, max_v, param):
    ...
    for i_frame in range(6):
        ...
        plt.axis("tight")
    fig.suptitle(
        "Strain of platinium nanoparticle " + param,
        fontweight="bold",
        horizontalalignment="center",
        y=1,
    )
    ...
    for i_scan in index_best_run_scans.keys():
        ...
        for i in index_best_run_scans[i_scan]:
            ...
            if param == "phi":
                ...
                im = ax.imshow(
                    mean_x_run(data_scans[i_scan][i]),
                    cmap="jet",
                    vmin=0,
                    vmax=6,
                )
    ...
    if plot_norm:
        ...
        if norm_values is None:
            norm_values_i_pt = np.sqrt(
                np.array(com_values[0]) ** 2
                + np.array(com_values[1]) ** 2
                + np.array(com_values[2]) ** 2
            )
        else:
    ...
    for i_pt in range(num_pts):
        ...
        if plot_norm:
            if norm_values is None:
                ...
                norm_values_i_pt = norm_values[i_pt]
                if filter_zero_value:
                    norm_values_i_pt = np.array(norm_values_i_pt)[
                        non_zero_indices
                    ]  # type: ignore
            if Normalize_parametre:
...
def plot_combined_single_coordinate(
    com_values,
    scan_values,
    axis_label,
    f_s=16,
...
def plot_3darray_as_gif_animation(data, file_save, vmin, vmax, title_fig=""):
    ...
    ani = animation.FuncAnimation(
        fig, update, frames=data.shape[2], interval=100
    )  # type: ignore
...
def summary_slice_plot_abd(
    ...
    phase_min_max=None,
    displacement_min_max=None,
    het_strain_min_max=None,
    **kwargs,
    ...
    **kwargs : dict
    ...
    table = table_ax.table(
        cellText=np.transpose(
            [
                [str(voxel_size).replace("[", "").replace("]", "")],
                [format_vector(det_reference_voxel)],
                [isosurface],
                [averaged_dspacing],
...
def plot_3darray_slices_as_subplots(
    ...
    # Set up the animation
    ani_xy = animation.FuncAnimation(
        fig, update_xy, frames=data.shape[1], interval=100
    )  # type: ignore
...
def plot_single_3darray_slices_as_subplots(
    data, file_prefix, vmin, vmax, title_fig="", proj=1, dpi=150, fps=5
):
    import matplotlib.animation as animation
    import matplotlib.pyplot as plt
    ...
    # Set up the animation
    ani_xy = animation.FuncAnimation(
        fig, update_xy, frames=data.shape[0], interval=100
    )  # type: ignore
...
def plot_X_vs_Y_allpart_or_onebyone(
    X,
    Y,
    part_name_list,
    save_dir_plot=None,
...
def plot_data_lattice_parametre_multidata(
    x_label,
    y_label,
    fig_title,
    subtitles,
    ...
    a_max,
    nov_2022_temp_scan,
    nov_2022_lattice_parametre,
    nov_2022_part_name_list,
    Avril_2023_temp_scan,
    Avril_2023_lattice_parametre,
    Avril_2023_part_name_list,
    Jan_2024_temp_scan,
    Jan_2024_lattice_parametre,
    Jan_2024_part_name_list,
    marker="+",
    show_plt=True,
    save_dir_plot=None,
):
    ...
    for i in range(3):
        ...
        for particle in np.unique(all_part_name_lists[i]):
            x_plot = [
                x
                for x, p in zip(all_part_temp_lists[i], all_part_name_lists[i])
                if p == particle
            ]
            y_plot = [
                y
                for y, p in zip(all_part_y_lists[i], all_part_name_lists[i])
                if p == particle
            ]
            plt.plot(
                x_plot,
                ...
                marker,
                label=particle,
                color=color_map[particle],
            )
        if i <= 1:
            desired_order = [
                ["27", "100", "370", "370 RT", "800 RT", "950 RT"],
...
def plot_data_single_or_multiple(
    x_label,
    y_label,
    fig_title,
    subtitles,
    ...
    def format_ticks_scientific(ax, axis="y", font_size=16):
        ...
        formatter.set_powerlimits(
            (-2, 2)
        )  # Use scientific notation for numbers outside these limits
    ...
    if norm:
        max_for_norm = np.array(
            [
                data_sets[i_dataset][i_exp][1].max()
                for i_dataset in range(len(data_sets))
                for i_exp in range(len(data_sets[i_dataset]))
            ]
        ).max()
        print()
        max_for_norm = (
            np.round(extract_coefficient_and_exponent(max_for_norm)[0], 2)
            * 10 ** extract_coefficient_and_exponent(max_for_norm)[1]
        )
    # Define color map for particles
    from itertools import combinations
                ...
                for i, data_set in enumerate(data_sets[i_datasets_pos]):
                    ...
                    if norm:
                        ...
                        if unit_y:
                            ...
                            plt.ylabel(
                                y_label_ + " " + y_labels[i_datasets_pos]
                            )  # type: ignore
                    plt.legend(loc="best", ncols=3)
                    if a_min and a_max:
                        plt.ylim(a_min, a_max)
        ...
        if xyz_plot:
            ...
            for particle in np.unique(unique_particles):
                ...
                fig, axes = plt.subplots(
                    nrows=1,
                    ncols=num_plots_loc,
                    figsize=(figsize_UNIT[0] * num_plots_loc, figsize_UNIT[1]),
                )
                for i_datasets_pos in range(3):
                    i_subplots = 0
                    for i, data_set in enumerate(data_sets[i_datasets_pos]):
                        X, Y, part_name_list = data_set
                        if (trigger_subplots[i] == 0) or (
                            particle not in part_name_list
                        ):
                            continue
                        if num_plots_loc == 1:
                            ax_loc = axes
                        ...
                        ax_loc.plot(
                            ...
                            marker,
                            ms=marker_size,
                            color=colors[i_datasets_pos],
                            label=y_labels[i_datasets_pos],
                            linestyle=linestyle,
                            linewidth=linewidth,
                        )  # type: ignore
                        ...
                        i_subplots += 1
                plt.suptitle(fig_title + " " + particle, ha="center")
                plt.tight_layout()
            ...
            for particle in np.unique(unique_particles):
                ...
                for i, data_set in enumerate(data_sets):
                    ...
                    if len(y_plot) == 0:
                        ...
                        i_subplots += 1
                plt.suptitle(
                    fig_title + " " + particle, x=i_subplots * 0.5 / (i + 1)
                )  # type: ignore
                plt.tight_layout()
                if save_dir_plot:
                    plt.savefig(
...
def plot_stast_evolution_id27(
    x_absis,
    stats_x,
    stats_y,
    stats_z,
    pressure_allscan_list,
    y_label="",
    y_label_unit="",
    label_rot=-70,
    fontsize_ticks=60,
    ...
    def figure_axes_desidn(ax, label_rot):
        ...
        formatter.set_powerlimits(
            (-2, 2)
        )  # Show scientific notation if values are very small/large
        ax.yaxis.set_major_formatter(formatter)
    ...
    list_presure_to_show = np.array(
        ["0.3", "1.6", "4.5", "4.6", "", "5", "5.0", "", "5.6", "6.7"]
    )
    ...
    def add_colored_bands(ax):
        for i in range(len(x_absis)):
            i_pres = x_absis[i].split()[0][
                1:
            ]  # Assuming pressure is the first part before a space
    ...
    if len(zoom_indices) > 0:
        ax_zoom = fig.add_axes([0.64, 0.14, 0.15, 0.0005])  # type: ignore
        ...
        float_labels = [
            "4.77",
            "5.2",
        ]  # array([f"{float(label):.2f}" for label in list_presure_to_show_p[zoom_indices]]).astype(str)
        ...
        ax_zoom.set_xlim(4.0, 9.0)  # Set y-limits to create some space
        ax_zoom.axis("tight")
...
def anealing_plot_stat_multiple(
    temperatures,
    stat_params_groups,
    particle_names,
    stat_param_names=None,
    desired_order=None,
    line_style="-",
    linewidth=4,
    markersize=15,
    ...
    particle_to_color = {
        particle: color_map(i) for i, particle in enumerate(unique_particles)
    }
    marker_list = ["o", "s", "^", "d", "v", "<", ">", "P", "X", "*"]
    particle_to_marker = {
        particle: marker_list[i % len(marker_list)]
    ...
    for i_group in range(n_groups):
        for i_coord in range(n_coords):
            ...
            coord = ["X", "Y", "Z"][i_coord]
            ax.set_title(
                f"{stat_param_names[i_group]} {coord}",
                fontsize=font_size,
                fontweight="bold",
            )
            ax.grid(True)
...
def edit_colorbar(mappable, scale="linear", numticks=10, label=None, pad=0.05):
    """
...
def contour_slices(
    ...
    if not sum_frames:
        slice_position = slice_position or (
            int(nbz // 2),
            int(nby // 2),
            int(nbx // 2),
        )
    ...
    **kwargs,
    ...
    for idx in range(nb_subplots):
        ...
        if invert_yaxis:  # Y is axis 0, need to be flipped
            axis.invert_yaxis()
        if plot_colorbar:
            cbar = edit_colorbar(plot, numticks=5)
