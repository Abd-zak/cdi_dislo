...
def log_scale_matteotype_renormalization(data, plot=False):
    """
    ...
    data_log = (data_log - np.nanmin(data_log)) / (
        np.nanmax(data_log) - np.nanmin(data_log)
    )
    data_log[mask_zeros] = 0.0
...
def mask_smoothing(mask, size_filter=5, plot=False, data=None):
    """
...
def filter_out_labels(pixels, pixels_labels, labels_to_remove):
    """
...
def keep_only_largest_clusters(pixels, pixels_labels, nb_clusters_kept=10, plot=False):
    ...
    if plot:
        ...
        plt.axvline(x=len(labels) - nb_clusters_kept, color="r", linewidth=0.5)
...
def compute_asymmetry_matrix(data, plot=False):
    """
    ...
    asym = 2.0 * np.abs(data - data_inv) / (data + data_inv)
...
def plot_2d_projection_cluster(data, pixels, pixels_labels, label, fig=None, ax=None):
    ...
    plot_3D_projections(
        data, log_scale=True, mask=mask, fig_title=fig_title, fig=fig, ax=ax
    )
    return
...
def plot_clusters_select_2d_projections(data, pixels, pixels_labels, sorting="size"):
    ...
    if sorting == "asym":
        ...
        raise ValueError(
            "sorting possible values are 'size', 'max', 'asym' and 'None' "
        )
...
def create_mask_from_selected_clusters(
    data, check_list, pixels, pixels_labels, labels, plot=False
):
    """
    ...
    if plot:
        ...
        plot_3D_projections(
            data, log_scale=True, mask=mask_alien, fig_title="data with mask"
        )
...
###########################################################################################################################################
def preprocessing(data, remove_hot_pixels=True, filter_small_values=True):
    """
...
def clustering_and_filtering(mask, data, nb_clusters_kept=10, sorting="size"):
    """
    ...
    pixels, pixels_labels = keep_only_largest_clusters(
        pixels, pixels_labels, nb_clusters_kept=nb_clusters_kept
    )

    print("number of clusters to plot :", len(np.unique(pixels_labels)))
    # Cluster selection
    check_list, labels = plot_clusters_select_2d_projections(
        data, pixels, pixels_labels, sorting=sorting
    )
...
def get_clean_data(
    ...
    if replace_by_zeros:
        data_clean[mask_alien == 1.0] = 0.0
    else:
        data_clean = data_clean.astype("float64")
        data_clean[mask_alien == 1.0] = np.nan
