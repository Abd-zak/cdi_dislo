...
def sort_objects(obj_list, support_list, plot=False):
    ...
    if plot:
        for n in range(len(obj_list)):
            plot_2D_slices_middle(
                obj_list[n], fig_title=f"sharpness : {metric_list[n]}"
            )
...
def press_button_genetic(
    data, nb_recon, nb_genetic, mask=None, params_init=None, plot=True
):
    obj_list = None
    support_list = None
    ...
    for n_gen in range(nb_genetic):
        ...
        obj_list, support_list = make_several_reconstruction(
            data,
            nb_recon,
            params_init,
            obj_list=obj_list,
        ...
        metric_list, obj_list, support_list = sort_objects(
            obj_list, support_list, plot=False
        )
    ...
    if plot:
        plot_2D_slices_middle(
            ...
            fig_title="Result of genetic algorithm",
        )
    return obj_result
...
def save_reconstruction_genetic(
    ...
    path_save = "Reconstructions_Genetic/{}_scan{}{}{}/".format(
        file_dict["h5file"],
        file_dict["scan_nb"],
        ortho_string,
...
def make_several_reconstructions_genetic(
    data,
    file_dict,
    nb_recon,
    nb_genetic,
    ...
    for n_reconstruction in range(Nb_reconstruction):
        ...
        save_reconstruction_genetic(
            file_dict, obj, llk, support, last_recon_nb + 1 + n_reconstruction
        )
    return
