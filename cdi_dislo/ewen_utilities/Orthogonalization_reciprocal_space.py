...
from cdi_dislo.ewen_utilities.Global_utilities import get_npz_files
...
def QSpaceTransformation_orthogonalization(data, qx, qy, qz, return_qs=False):
    maxbins = []
    for dim in (qx, qy, qz):
...
def compute_automatic_voxel_sizes(qx, qy, qz):
    ...
    for n, q in enumerate([qx, qy, qz]):
        ...
        voxel_sizes[n] += 2.0 * np.pi / range_q
    return voxel_sizes


def OrthogonalizeObject(obj, path_reconstruction, plot=False):
    ...
    I_exp = np.load(str(file["preprocessed_datapath"]))[
        "data"
    ]  # experimental intensity
    data_recon = fftshift(fftn(ifftshift(obj)))  # reconstructed amplitude

    data_module_ortho, qx1d_ortho, qy1d_ortho, qz1d_ortho = (
        QSpaceTransformation_orthogonalization(
            np.abs(data_recon), qx, qy, qz, return_qs=True
        )
    )
    data_phase_ortho = QSpaceTransformation_orthogonalization(
        np.angle(data_recon), qx, qy, qz
    )
    ...
    if plot:
        ...
        plot_3D_projections(np.abs(data_recon) ** 2.0, fig=fig, ax=ax[1])
        plot_3D_projections(np.abs(data_ortho) ** 2.0, fig=fig, ax=ax[2])
    ...
    if plot:
        ...
        fig.tight_layout()
        fig, ax = plot_2D_slices_middle(
            obj_ortho, return_fig_ax=True, threshold_module=0.3
        )
...
def interactive_threshold_selection(obj, axis=0):
    ...
    @interact(threshold=widgets.FloatSlider(min=0, max=0.5, step=0.005))
    def update(threshold=0):
        module_clean = np.copy(module)
        phase_clean = np.copy(phase)

        module_clean[module < threshold * np.max(module)] = 0.0
        phase_clean[module < threshold * np.max(module)] = np.nan

        fig.suptitle("threshold : {}".format(threshold))
        for axis in range(3):
            im_module[axis].set_data(
                module_clean.take(indices=shape[axis] // 2, axis=axis)
            )
            im_phase[axis].set_data(
                phase_clean.take(indices=shape[axis] // 2, axis=axis)
            )
