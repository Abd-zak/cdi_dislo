...
def create_Q_array(
    ...
    hxrd.Ang2Q.init_area(
        ...
        "y+",
        cch1=beam_center_y - roi[2],
        cch2=beam_center_x - roi[4],
        Nch1=roi[3] - roi[2],
        Nch2=roi[5] - roi[4],
        pwidth1=det_calib["y_pixel_size"],
        pwidth2=det_calib["x_pixel_size"],
    ...
    if verbose:
        if switch_x_y_direct_beam_position:
            print(
                "\x1b[31m beam_center_x and beam_center_y were switched when loading the detector calibration parameters ! \x1b[0m"
            )
            print("\n")
    ...
    if cxi_convention:
        qy, qz = qz, qy
        print(
            "qy and qz were switched in order to end up with the good orientation for the VTI"
        )
    return qx, qy, qz
...
def Q_space_transformation(
    ...
    if plot:
        ...
        plot_3D_projections(
            data_q_space, fig_title="orthogonalized data", colorbar=True
        )
...
def center_of_mass_calculation_two_steps(data, crop=50, plot=False):
    ...
    cropping_dim0 = (
        max([0, int(piz - crop / 2)]),
        min(int(piz + crop // 2), data.shape[0] - 1),
    )
    cropping_dim1 = (
        max([0, int(piy - crop / 2)]),
        min(int(piy + crop // 2), data.shape[1] - 1),
    )
    cropping_dim2 = (
        max([0, int(pix - crop / 2)]),
        min(int(pix + crop // 2), data.shape[2] - 1),
    )

    piz2, piy2, pix2 = center_of_mass(
        data[
            cropping_dim0[0] : cropping_dim0[1],
    ...
    piz, piy, pix = (
        int(round(cropping_dim0[0] + piz2)),
        int(round(cropping_dim1[0] + piy2)),
        int(round(cropping_dim2[0] + pix2)),
    )
...
def automatic_roi_selection(
    ...
    crop=False,
    crop_array=[0, 0, 0],
    crop_with_final_size=False,
    final_size_array=None,
    plot=False,
):
    data = data[
        roi_init[0] : roi_init[1],
        roi_init[2] : roi_init[3],
        roi_init[4] : roi_init[5],
    ]
    ...
    if crop_with_final_size:
        roi = crop_roi_given_final_size(
            data, roi, final_size_array, plot=plot, fig_title="cropped"
        )
...
def crop_roi(data, roi, crop_array, plot=False, fig_title=None):
    roi_crop = [
        roi[0] + crop_array[0] // 2,
        roi[1] - crop_array[0] // 2,
        roi[2] + crop_array[1] // 2,
        roi[3] - crop_array[1] // 2,
        roi[4] + crop_array[2] // 2,
        roi[5] - crop_array[2] // 2,
    ]
    if plot:
        from cdi_dislo.ewen_utilities.plot_utilities import plot_3D_projections
...
def crop_roi_given_final_size(
    ...
    roi_crop = [
        roi[0] + (roi[1] - roi[0] - final_size_array[0]) // 2,
        roi[1] - (roi[1] - roi[0] - final_size_array[0]) // 2,
        roi[2] + (roi[3] - roi[2] - final_size_array[1]) // 2,
        roi[3] - (roi[3] - roi[2] - final_size_array[1]) // 2,
        roi[4] + (roi[5] - roi[4] - final_size_array[2]) // 2,
        roi[5] - (roi[5] - roi[4] - final_size_array[2]) // 2,
    ]
...
def find_1D_center_custom(array1d, x=None, remove_min=True):
    if x is None:
...
def find_center_custom(detector_sum, roi_init=None):
    if roi_init is None:
        roi_init = [0, detector_sum.shape[0] - 1, 0, detector_sum.shape[1] - 1]

    # Find center in vertical
    projection = np.sum(
        detector_sum[roi_init[0] : roi_init[1], roi_init[2] : roi_init[3]],
        axis=1,
    )
...
def automatic_roi_selection_2D_BCDI_data(
    detector_sum, roi_size_vertical, roi_size_horizontal, roi_init=None
):
    cen_vert, cen_hori = find_center_custom(detector_sum, roi_init=roi_init)
    roi = [
        round(cen_vert) - roi_size_vertical // 2,
        round(cen_vert) + roi_size_vertical // 2,
        round(cen_hori) - roi_size_horizontal // 2,
        round(cen_hori) + roi_size_horizontal // 2,
    ]
...
def save_preprocessed_data(
    ...
    save_name = path_save + "dataset_{}_scan_{}{}{}".format(
        dataset_name, scan_nb, ortho_string, savename_add_string
    )

    # overwrite_safety
    if os.path.exists(save_name + ".npz"):
        overwrite = input(
            f"{save_name}.npz already exist ! This is an overwriting safety."
...
def plot_3D_projections_qcen(data, qx, qy, qz, qcen, suptitle=None):
    from cdi_dislo.ewen_utilities.plot_utilities import plot_3D_projections

    q = np.array([qx, qy, qz])
    rough_cen_index = np.unravel_index(
        np.argmin(np.sum((q - qcen[:, None, None, None]) ** 2.0, axis=0)),
        data.shape,
    )
...
def calculate_Q_center_of_mass(data, qx, qy, qz, remove_min=True, plot=False):
    if remove_min:
        data -= np.nanmin(data)
...
def calculate_qmax(data, qx, qy, qz, plot=False):
...
def correct_flatfield(data, roi, flatfield_file, plot=False):
...
def compute_oversampling_ratio(support, plot=False):
    """
...
def oversampling_from_diffraction(
    data, support_threshold=0.1, plot=False, verbose=True
):
    """
    ...
    if verbose:
        ...
        print(
            "oversampling after rebinning : ",
            np.divide(oversampling, rebinning_possibility),
        )
...
def load_mask(scan, data, roi=None, plot=False):
    ...
    if plot:
        ...
        for n in range(3):
            ax[n].matshow(
                xu.maplog(((1 - mask) * data).sum(axis=n), 5, 0), cmap=my_cmap
            )
...
def force_even_dimension(data, qx, qy, qz, mask, verbose=False):
    s = []
    for n in range(data.ndim):
        if data.shape[n] % 2 == 0:
            s.append(slice(None))
