...
from cdi_dislo.ewen_utilities.plot_utilities import (
    ...
    compare_reconstuction_to_real_data,
)
from cdi_dislo.ewen_utilities.Object_utilities import (
    get_cropped_module_phase,
    center_object_list,
    get_complex_conjugate,
    compute_oversampling_ratio,
)
...
def compute_module_max_position_metric(obj, bins=None):
    if bins is None:
        if obj.ndim == 2:
            bins = 30
        if obj.ndim == 3:
...
def compute_module_histogram_peak_width(obj, plot=False):
    module, phase = get_cropped_module_phase(obj)
...
###########################################################################################################################################
def automatic_best_objects_selection(
    path_reconstruction,
    nb_keep_llk=6,
    nb_keep=4,
    ...
    for n in range(len(obj_list)):
        ...
        try:
            ...
            n_hist, bins = np.histogram(
                module[np.logical_not(np.isnan(module))], bins=100
            )
...
def pearson_correlation_coef(img1, img2):
    ...
    denominator = np.sqrt(np.sum(img_corr[0] ** 2.0)) * np.sqrt(
        np.sum(img_corr[1] ** 2.0)
    )
...
def mode_decomposition(obj_list, plot=False):
...
def remove_phase_linear_fit(phase):
    ...
    #     ramp = -ramp
    ramp = (
        np.sum([reg.coef_[n] * pos[+n] for n in range(len(pos))], axis=0)
        + reg.intercept_
    )
    return ramp
...
def remove_phase_ramp(
    ...
    module, phase = get_cropped_module_phase(
        obj, crop=crop, unwrap=True, threshold_module=threshold_module
    )
    ...
    _, phase_full = get_cropped_module_phase(
        obj, crop=crop, unwrap=True, threshold_module=0.0
    )
    ...
    if plot:
        if obj.ndim == 2:
            ...
            module, phase = get_cropped_module_phase(
                obj, crop=crop, threshold_module=threshold_module
            )
            module_no_ramp, phase_no_ramp = get_cropped_module_phase(
                obj_no_ramp, crop=crop, threshold_module=threshold_module
            )
            plt.figure()
...
def blackman_window(shape):
    ...
    for ii, n in enumerate(index):
        blackman = blackman * (
            0.42
            - 0.5 * np.cos(2.0 * pi * n / (shape[ii] - 1))
            + 0.08 * np.cos(4.0 * pi * n / (shape[ii] - 1))
        )
    blackman = blackman / np.sum(blackman)
    return blackman


def apodize(obj, window_type="blackman", plot=False):
    shape = obj.shape
    if window_type == "blackman":
...
def combine_reconstructions(
    ...
    file_ref = np.load(
        file_list[index_best_recon[0]]
    )  # file as reference use for later strain calculation
    ...
    if conj_index == "auto":
        conj_index = automatic_conj_index_finding(
            file_list, index_best_recon, conj_index_start=conj_index_start
        )

    if conj_index is None:
        obj_list = force_same_complex_conjugate_object_list(
            obj_list
        )  # Not working very well for 3D data
    else:
        for n in range(len(obj_list)):
            if conj_index[n] == 1:
                #                 obj_list[n] = np.conj(obj_list[n][::-1,::-1,::-1])
                obj_list[n] = np.conj(
                    np.flip(obj_list[n], axis=range(obj_list[n].ndim))
                )
    ...
    if apodization:
        obj = apodize(obj, window_type="blackman", plot=plot)
    ...
    if plot_result or plot:
        if obj.ndim == 3:
            plot_2D_slices_middle(obj, threshold_module=0.3)
        elif obj.ndim == 2:
...
def remove_bad_reconstructions(file_list, index_best_recon, verbose=False):
