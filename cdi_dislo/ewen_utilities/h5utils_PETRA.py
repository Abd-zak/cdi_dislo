...
class Scan:
    ...
    def getEnergy(self):
        with h5.File(self.master_h5file, "r") as h5f:
            energy = (
                12.39842 / h5f["entry/instrument/beam/incident_wavelength"][()]
            ) * 1e3  # energy in eV
        self.energy = energy
        return energy
    ...
    def getAllMotorDictionary(self):
        ...
        for line in lines[n:]:
            if "! Acquisition ended" not in line:
                for motor in column_dict.keys():
                    motor_dict[motor].append(
                        float(line.split()[column_dict[motor] - 1])
                    )
        return motor_dict
    ...
    def getImageRaw(self, apply_mask=True, roi=None):
        ...
        if "dmesh" in self.command:
            nb_elements = (int(self.command.split()[4]) + 1) * (
                int(self.command.split()[8]) + 1
            )
        if "scan" in self.command and "2scan" not in self.command:
            ...
            for ii, data_h5file in enumerate(self.data_h5file_list):
                with h5.File(data_h5file, "r") as h5f:
                    if ii == len(self.data_h5file_list) - 1:
                        data[ii * 2000 :] += h5f["entry/data/data"][
                            :, roi[0] : roi[1], roi[2] : roi[3]
                        ]
                    else:
