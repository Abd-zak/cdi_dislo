...
def gaussian_kernel(sigma=1.0):
    ...
    z = z - np.mean(z)
    r = np.sqrt(x**2.0 + y**2.0 + z**2.0)
    kernel = np.exp(-(r**2.0) / (0.5 * sigma**2.0))
    return kernel
...
def get_distance_to_array_borders(support):
    ...
    for n in range(3):
        distance_border_array[n] += min(
            np.min(indices_support[n]),
            (support.shape[n] - 1) - np.max(indices_support[n]),
        )
...
def padding_avoid_array_border_problems(
    ...
    for n in range(3):
        if (
            distance_border_array[n] < Nb_surface_pixels_added - 2
        ):  # I add this -2 for safety
...
def one_run(strain, support, kernel):
    ...
    strain_update = np.nansum(
        [strain, strain_frontier], axis=0
...
def several_run(strain, support, kernel, Nb_surface_pixels_added):
    ...
    strain_update[np.isnan(strain)] = (
        0.0  # I need to remove all nan's for my convolution.
    )
...
def add_surface_pixels(
    ...
    if Nb_surface_pixels_added is None:
        ...
        print(
            "problem, Nb_surface_pixels_added should be less than {}. I overwrite it.".format(
                max_pixels_added
            )
        )
    if verbose:
    ...
    strain_update, support_update = several_run(
        strain, support, kernel, Nb_surface_pixels_added
    )

    if plot:
        plot_2D_slices_middle_one_array3D(
            strain, cmap="bwr", fig_title="original strain"
        )
        ...
        plot_2D_slices_middle_one_array3D(
            strain_update,
            cmap="bwr",
            fig_title="strain with additional pixels",
        )
...
def save_vti(
    ...
    if output_path is None:
        output_path = path_reconstruction + "final_obj/{}.vti".format(
            path_reconstruction.split("/")[-2]
        )
...
def save_arrays_in_vti(
    np_arrays_dict, savename="default_vti_name.vti", origin=(0, 0, 0)
):
    if savename[-4:] != ".vti":
        savename += ".vti"
