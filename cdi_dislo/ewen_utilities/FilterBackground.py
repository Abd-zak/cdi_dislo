...
def filter_background_zlotnikov(
    data,
    ...
    profile = (
        np.nanmean(data[:nb_points_rocking], axis=(0, 1))
        + np.nanmean(data[-nb_points_rocking:], axis=(0, 1))
    ) / 2.0

    x = np.arange(len(profile))
    indices = np.concatenate(
        (np.arange(nb_points_start_end), -np.arange(nb_points_start_end))
    )
    ...
    if plot:
        ...
        ax[0, 0].plot(
            x_rock[:nb_points_rocking],
            np.log(rocking[:nb_points_rocking]),
            "r.-",
        )
        ax[0, 0].plot(
            x_rock[-nb_points_rocking:],
