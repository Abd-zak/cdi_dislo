...
def select_border_roi2D(obj_ortho, voxel_sizes, axis, roi):
...
def sigmoid_plus_background_2D(xy, A, x0, theta, sig, back):
    ...
    return back + A * 1.0 / (1.0 + np.exp(-factor / sig))


def fit_surface(border2D, voxel_sizes2D, verbose=True, plot=False):
    ...
    popt, pcov = curve_fit(
        sigmoid_plus_background_2D,
        (x.flatten(), y.flatten()),
        border2D.flatten(),
    )

    surface_width = 2.0 * np.abs(popt[-2])
    print("voxel sizes (nm) : ", 0.1 * voxel_sizes2D)
    print(f"surface width : {round(0.1 * surface_width, 3)} nm")
...
def Fourier_shell_correlation(fexp1, fexp2, q, N=30, plot=False):
    dq = (np.max(q) - np.min(q)) / N
    q1d = np.linspace(np.min(q), np.max(q) - dq, N) + dq / 2.0
    dq = q1d[1] - q1d[0]
    fsc = np.zeros(N)
    ...
    for n in range(N):
        ...
        denominator = np.sqrt(
            np.nansum(np.abs(fexp1[indices]) ** 2.0)
            * np.nansum(np.abs(fexp2[indices]) ** 2.0)
        )
        fsc[n] += np.abs(numerator / denominator)
    return fsc, q1d


def Fourier_correlation_shell_plot(
    fsc, q1d, threshold, resolution=None, prtf_plot=False
):
    ...
    if resolution is not None:
        ...
        ax1.text(
            ...
            ypos,
            f"resolution : {round(0.1 * resolution)} nm",
            fontsize=15,
            color="k",
        )
    return
...
def compute_Fourier_shell_correlation_two_object(
    obj1,
    obj2,
    qx,
    qy,
    ...
    qy = qy - qcen[1]
    qz = qz - qcen[2]
    q = np.sqrt(qx**2.0 + qy**2.0 + qz**2.0)
    if plot:
    ...
    if plot:
        plot_2D_slices_middle(
            ...
            fig_title=r"1$^{st}$ object",
            crop=False,
        )
        plot_2D_slices_middle(
            obj2,
            threshold_module=0.3,
            fig_title=r"2$^{nd}$ object",
            crop=False,
        )
...
def compute_resolution_Fourier_shell_correlation(
    obj_list,
    file_ref,
    index1=0,
    index2=1,
    ...
    fsc, q1d, resolution = compute_Fourier_shell_correlation_two_object(
        obj1,
        obj2,
        qx,
        qy,
...
def compute_resolution_Fourier_shell_correlation_orthogonalized_objects(
    obj_list,
    file_ref,
    index1=0,
    index2=1,
    ...
    obj_ortho1, voxel_sizes = real_space_orthogonalization(
        obj1, file_ref, final_roi=False, plot=False, verbose=False
    )
    obj_ortho2, voxel_sizes = real_space_orthogonalization(
        obj2, file_ref, final_roi=False, plot=False, verbose=False
    )
    ...
    qx = qx * 2.0 * pi / (voxel_sizes[0] * obj_ortho1.shape[0])
    qy = qy * 2.0 * pi / (voxel_sizes[1] * obj_ortho1.shape[1])
    qz = qz * 2.0 * pi / (voxel_sizes[2] * obj_ortho1.shape[2])
...
def Fourier_shell_average(array3D_qspace, q, N=30, plot=False):
    dq = (np.max(q) - np.min(q)) / N
    q1d = np.linspace(np.min(q), np.max(q) - dq, N) + dq / 2.0
    dq = q1d[1] - q1d[0]
...
def PTRF_resolution(
    ...
    data_recon = [
        np.abs(create_diffracted_amplitude(obj_list[n])) ** 2.0
        for n in range(len(obj_list))
    ]
    data_recon = np.nanmean(data_recon, axis=0)
    data_recon = (
        data_recon * np.max(data) / np.max(data_recon)
    )  # Is it a good idea to do this here instead of on prtf?
    ...
    if np.all(prtf > threshold):
        ...
        resolution = 2.0 * pi / q1d[np.where(prtf < threshold)[0][0]]
    print(f"\nreal space resolution : {round(0.1 * resolution, 2)} nm")

    if plot_prtf:
        Fourier_correlation_shell_plot(
            prtf, q1d, threshold, resolution=resolution, prtf_plot=True
        )
...
def Gaussian3D(N, sig):
    ...
    gaussian = np.exp(-0.5 * r**2.0 / sig**2.0)
    return gaussian
...
def fit_PSF_width(psf_deconv, voxel_sizes):
    ...
    for n in range(3):
        ...
        fit, popt, pcov = GaussianAndLinearBackgroundFit(
            proj,
            x=None,
            sig=None,
        ...
        title += f"\n resolution (nm) : {round(0.1 * resolution[n], 1)}"
        ax[n].set_title(title, fontsize=15)
        fig.suptitle(
            r"width is given by the gaussian variance ($\sigma$ parameter)",
            fontsize=20,
            y=1.2,
        )
...
def resolution_cherukara(
    ...
    if plot:
        plot_2D_slices_middle_one_array3D(
            psf_init, fig_title="initial start for the psf"
        )
        plot_2D_slices_middle_one_array3D(
            support_conv,
            fig_title=r"$\it{Convolution}$[ support, psf initial ]",
        )

    psf_deconv, deconv_error = richardson_lucy_mine(
        psf_init,
        module,
    ...
    if plot:
        ...
        plot_3D_projections(
            ...
            cmap="gray_r",
            fw=3,
            fig_title="PSF result",
        )

        support_conv = fftconvolve(support, psf_deconv, mode="same")
        plot_2D_slices_middle_one_array3D(
            support_conv,
            fig_title=r"$\it{Convolution}$[ support, psf result ]",
        )
        plot_2D_slices_middle_one_array3D(module, fig_title="object module")
